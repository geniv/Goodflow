 ...
 */
   ...
   * @version 1.18
   */
  class Sitemap { //TODO: unit testy, upravit komentare
    ...
    public function addImage($loc, $title = null, $caption = null, $geo_location = null, $license = null) {
      if ($this->last) {
        ...
        $image = $this->last->addChild('c:image:image');
        $image->addChild('c:image:loc', $loc);  // pridani loc
        if ($title) {         // pokud je title
          $image->addChild('c:image:title', htmlspecialchars($title));
        }
        if ($caption) {       // pokud je caption
          $image->addChild('c:image:caption', htmlspecialchars($caption));
        }
        if ($geo_location) {  // pokud je geo_location
          $image->addChild('c:image:geo_location', htmlspecialchars($geo_location));
        }
        if ($license) {       // pokud je license
          $image->addChild('c:image:license', htmlspecialchars($license));
        }
      }
      return $this;
    }
