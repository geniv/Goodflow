...
//R3: firma, jednorazova_sleva, balicek, balicek_nabidka_her
    ...
    public function getNovinky($mini = true, $id = 0, $limit = '0, 3') {
      if ($mini) {
        // podle limitu
        return $this['db']->query('novinky', array('idnovinky', 'idspravce', 'nadpis', 'pridano'), null, null, null, null, 'pridano DESC', $limit);
      } else {
        // podle id
        $where = ($id > 0 ? 'idnovinky=?' : null);
        $whereargs = ($id > 0 ? array($id) : null);
        return $this['db']->query('novinky', array('idnovinky', 'idspravce', 'nadpis', 'zprava', 'pridano'), $where, $whereargs, null, null, 'pridano DESC');
      }
    }
...
//TODO ještě tu musí byt tabulka pro jazyky aby se jednotlive zeme daly zahrnout do prekladu
  ...
  /**
   * trida pro autorizaci uzivatelu
   *
   * @package unstable
   * @author geniv
   * @version 1.00
