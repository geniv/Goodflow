 ...
 */
  ...
  final class Tpl implements ICron {
    ...
    private static $config = array(
      ...
      'black_list' => array(  // TODO black_list je tu zatim jen ke skode...
        'exec', 'shell_exec', 'pcntl_exec', 'passthru', 'proc_open', 'system', 'posix_kill', 'posix_setsid', 'pcntl_fork', 'posix_uname', 'php_uname',
        'phpinfo', 'popen', 'file_put_contents', 'rmdir', 'mkdir', 'unlink', 'highlight_contents', 'symlink', 'apache_child_terminate',
      ...
      'tags' => array(  //TODO testy na vuceradkove variace!!!!!
                      'loop' => array('({loop.*?})', '/{loop="(?<variable>\$?[^"]*)"(?: as (?<key>\$.*?)(?: => (?<value>\$.*?))?)?}/s'), //ok, smycka
                      ...
                      'comment' => array('({\*|{\/\*)', '/{\*|{\/\*/'), //ok, komentar, {* nebo {/*
                      'comment_close' => array('(\*}|\*\/})', '/\*}|\*\/}/'), //ok, *} nebo */}
                      'include' => array('({include.*?})', '/{include="([^"]*)"}/'),  //ok, vkladani dalsiho tpl, fyzicke vlozeni do kodu
                      'compile' => array('({compile.*?})', '/{compile="([^"]*)"(?:,(?<argv>.*?))?}/'),  //ok, vkladani kodu na kompilaci
                      'compile_file' => array('({compile_file.*?})', '/{compile_file="([^"]*)"(?:,(?<argv>.*?))?}/'),  //ok, vkladani kodu na kompilaci ze souboru
                      'function' => array('({function.*?})', '/{function="([a-zA-Z_][a-zA-Z_0-9\\\:]*)(\(.*\))?"}/'), //ok, vkladani funkce
                      'date' => array('({date.*?})', '/{date="(?<format>.*?)",?(?<timestamp>.*?)?}/'), //ok, vkladani data
                      'date_str' => array('({date_str.*?})', '/{date_str="(?<format>.*?)",?(?<timestamp>.*?)?}/'), //ok, vkladani textoveho data
                      'variable' => array('({\$.*?})', '/{(\$.*?)}/s'),  //ok, promenne
                      'constant' => array('({#.*?})', '/{#(.*?)#?}/'),  //ok, konstanty
      ...
      ),//FIMXE zkontrolovat pattern: date & date_str !! + odzkouset: compile & compile_file
    );
     ...
     */
    public static function compile($tplcode, $assign = null, $removeBackslash = false) {
      $tpl = new self;  // konfigurace neni potreba
      $temp_file = tempnam('/tmp', 'tpl-'); // vytvoreni temp cesty
      if ($removeBackslash) { // odstranovani zpetnych lomitek
        $tplcode = str_replace(array('\{', '\}'), array('{', '}'), $tplcode);
      }
    ...
    private function compileTemplate($template) {
      ...
      if ($codeSplit) {
        ...
        foreach ($codeSplit as $html) {
          ...
          if ($commentIsOpen) { // vyhazuje kontext
            //~ var_dump($html);
          } else
          ...
          if ($noparseIsOpen) { // vkladani kontextu
            $result .= $html; // vlozeni
          } else
...
//TODO z include nebere backslash: {{loginForm.input.\$valid}} !!!
            if (preg_match('/.*\$.*/', $_path)) {
              // pokud se vklada promenna nebo funkce (+ hlidani existence pathu)
              $result .= "\nT\n. (file_exists('".self::$config['tpl_dir']."'.$_path.'.".self::$config['suffix']."') ? self::draw($_path)->assign(compact('".implode("', '", array_keys($this->vars))."'))->render() : '**<strong>'.$_path.'</strong> nelze najit**') . <<<T\n";
            } else {
          ...
          if (preg_match($tagMatch['compile'], $html, $matches)) {  // z TPL neumi I/O predavani promennych
            $removeBackslash = 'false'; //TODO pripadne pridat moznost zadavat pole a zadavat jednim parametrem i promenne ktere se maji pri kompilaci predat dovnitr!!!!
            //~ var_dump($matches);
            if (isset($matches['argv'])) {
              $removeBackslash = $matches['argv'];
            }
            $result .= "\nT\n. self::compile(".$matches[1].", compact('".implode("', '", array_keys($this->vars))."'), $removeBackslash) . <<<T\n";
          } else

          // zpracovani compile_file
          if (preg_match($tagMatch['compile_file'], $html, $matches)) {  // z TPL neumi I/O predavani promennych
            $removeBackslash = 'false'; //TODO pripadne pridat moznost zadavat pole a zadavat jednim parametrem i promenne ktere se maji pri kompilaci predat dovnitr!!!!
            //~ var_dump($matches);
            if (isset($matches['argv'])) {
              $removeBackslash = $matches['argv'];
            }
            $result .= "\nT\n. self::compile(file_get_contents('".self::$config['tpl_dir'] . $matches[1]."'), compact('".implode("', '", array_keys($this->vars))."'), $removeBackslash) . <<<T\n";
          } else
    ...
    /**
     * kompilace Tpl promenne
     *
     * @since 4.58
     * @param string html vstupni tpl promenna "$var.index"
     * @return string prekonvertovana hodnota "$var['index']"
     */
    public static function compileVar($html) {
      ...
      if (preg_match_all('/(\$[a-z_A-Z][^\s]*)/', $html, $matches)) {
        ...
        if (!preg_match('/\$.*=.*/', $html)) {
          ...
          if ($output) {  //nejaky vypocet
            if (preg_match('/(.*)\?(.*)\:(.*)/', $html)) {  // ? : - detekce a vlozeni ternaru, jen z promenne a jen nektere texty
              $html = "\nT\n. (" . $html . ") . <<<T\n";  // vlozeni
            } else {
              $html = "\nT;\n  (" . $html . '); '.$cmrs." .= <<<T\n"; // rozdeleni
            }
    ...
    /**
     * nacteni jmena kompilace
     * - volani az za ->template(xxx)
     *
     * @since 4.60
     * @param string|null template_name rucne zadane jmeno template, pokud neni zadane bere z metody ->template(xxx)
     * @return string jmeno kompilovaneho souboru
     */
    public function getCompileName($template_name = null) {
      $result = null;

      $template_name = $template_name ?: $this->template_name;

      // uplny path template
      $name = self::$config['tpl_dir'] . $template_name . '.' . self::$config['suffix'];

      if (file_exists($name)) { // pripraveni kompilacni cesty
        // slozeny link z hashu a nazvu templatu pro ulozeni kompilace
        $result = self::$config['compile_dir'] . str_replace('/', '__', $template_name) . '.' . md5_file($name) . '.php';
      }
      return $result;
    }
    ...
    public function render() {
      ...
      $_compile_path = $this->getCompileName($this->template_name);

      // pokud path kompilace neexistuje bude se ho pokouset zkompilovat, musi kompilovat pokud compile path neplati
      if (!file_exists($_compile_path) || self::$config['force_compile']) {
        // obsah pro kompilci se predava parametrem
        $compile = $this->compileTemplate(file_get_contents(self::$config['tpl_dir'] . $this->template_name . '.' . self::$config['suffix']));  // kompilace
