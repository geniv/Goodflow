 ...
 */
  ...
  class Emailer {
     ...
     */
    public static function factory($content = self::PLAIN, $charset = 'utf-8') {
    ...
    private function getFromatedEmail($email, $name = null) {
      return trim(($name ? $this->codeValue($name) : null) . ' <' . $email . '>');
    }
    ...
    /**
     * kodovani textu podle rcf normy
     * - RFC 2822 (http://tools.ietf.org/html/rfc2822)
     *
     * @since 4.18
     * @param string value vstupni hodnota
     * @return string zakodovany text podle specilaniho formatu
     */
    private function codeValue($value) {
      return '=?'.$this->charset.'?Q?'.imap_8bit($value).'?=';
    }
