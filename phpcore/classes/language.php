 ...
 */
  ...
  final class Language {
    ...
    /**
     * je jazykovy kod v poli mutaci?
     * 
     * @since 2.64
     * @param string code jazykovy kod na testovani
     * @return bool true pokud kod existuje
     */
    public function inListLanguage($code) {
      return array_key_exists($code, $this->list_language);
    }
     ...
     * - musi se volat pred init, vetsinou jako prvni
     *
     * @since 1.00
...
"X-Poedit-SearchPath-0: .\n"';
     ...
     * - vola se na konec, jako posledni
     *
     * @since 1.00
     * @param string path nova cesta pro umisteni jazykovych prekladu
     * @param string enabledLocaleException true pro povoleni exceptionu kdyz chyby systemova podpora jazyka
     * @return this
     */
    public function initLanguage($path = null, $enabledLocaleException = true) {
      if ($this->isChanged()) {
        ...
        if (file_exists($po_file) && file_exists($mo_file)) {
          ...
          if (!$this->status['setlocale'] && $enabledLocaleException) {
