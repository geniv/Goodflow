 ...
 */
  ...
  class TplForm {
    ...
    /**
     * nacitani poctu instanci
     * - v podstate jen pro informativni ucely
     *
     * @since 3.02
     * @param void
     * @return int realny pocet instanci
     */
    public static function getCountForm() {
      return self::$cnt_form - 1;
    }
    ...
    private function processAttr($match) {
      ...
      return str_replace(array('%%tpl__%%', '%%__tpl%%'), //TODO pripadne rozsirit??? nebo dat modifikovatelne?
                          array('{', '}'),  //FIXME nahraditpodle vzoru latte {l}, {r}
                          implode('', $_attr));
    }
...
//TODO metodu na skladani multi podminek respekzive neco jako addCondition|addConditionOn($name, $type, $args = null) {} ?????
     ...
     * @return this
     */
    public function addRule($name, $type, $text = null, $args = null) { //TODO oddelit do samostatne tridy...
      $callback = null;
      if (is_callable($type)) {
        $callback = $type;
    ...
    private function checkRules($rules) {
      ...
      switch ($rules['type']) {
        ...
        case 'image': // kontrola na obrazek (png, jpg, gif)
          ...
          if (is_array($value['name']) ? $value['name'][0] : $value['name']) {
            if (is_array($value['name'])) { // zpracovani multiple
              foreach ($value['type'] as $index => $_type) {
                if (!in_array($_type, $images) || !getimagesize($value['tmp_name'][$index])) { // pokud neni obrazek nebo neni typu obrazek
                  return false;
                }
              }
              return true;
            } else { // zpracovani single
              return in_array($value['type'], $images) && getimagesize($value['tmp_name']);
            }
          }
          return true;
     ...
     * - umi zpracovat pole hodnot (pri raw = false)
     *
     * @since 2.48
    ...
    private function prepareRaw($raw, $values) {
      if (!$raw) {
        ...
        if (is_array($values)) {
          foreach ($values as $i => $v) { // prochazeni pri multiple hodnotach
            $values[$i] = is_array($v) ? array_map($htmlspecial, $v) : $htmlspecial($v);  // zpracovava i pole
          }
        } else {
          $values = $htmlspecial($values);  // klasicka hodnota
        }
      }
      return $values;
    }
