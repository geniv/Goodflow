<?php
  ...
  class Domeny_Home implements IPage {
    //nazev a ruzne promenne
    public static function getName() {
      return array('name' => 'Výpis domén',
                  'name_blok' => 'Správa domén',
                  );
    }
...
//~ JS
    ...
    //~ public static function getSubmenu() {
      //~ return array('domena' => '_admin\pages\Faktury_Home');
    //~ }
...
//~ var_dump($uri);
      ...
      $f->addText('domena_nazev', 'nazev domeny', null, array('maxlength' => 100,))
          ->setRequired('Je nutne vyplnit nazev')
          ->setMaxLength('maximální délka je %s', 100)
        ->addText('domena_url', 'url domeny', null, array('maxlength' => 100,))
          ->setRequired('Je nutne vyplnit url')
          ->setMaxLength('maximální délka je %s', 100)
        ->addText('domena_vlastnik', 'email vlastníka co bude uvedena v konfiguraci', null, array('maxlength' => 100,))
          ->setRequired('Je nutne vyplnit email')
          ->setMaxLength('maximální délka je %s', 100)
        ->addDate('domena_od', 'Datum pořízení', date('Y-m-d'))
          ->setRequired('Je nutne vyplnit datum od')
        ->addDate('domena_do', 'Datum expirace', date('Y-m-d'))
          ->setRequired('Je nutne vyplnit datum do')
        ;
...
//~ 
        ...
        case 'add': // pridani aktuality
          ...
          $f->addBackLink('zpět', $backUrl)
            ->addSubmit('add_domena', null, 'Přidat doménu');
          ...
          if ($f->isSubmitted()) {
            ...
            if ($f->isValid()) {
              $val = $f->getValues();
///TODO dopsat obsluhu pridavani+vypis+dalsi 
              //~ $c = new ContentValues;
...
//~ //TODO pro CRONa vystup hodnot ktere podlehaji zkaze, expirace uzivatelu respektive smazanych udaju!
      ...
      return $tpl->template('serveradmin/domeny_home')->render();
    }
  }
