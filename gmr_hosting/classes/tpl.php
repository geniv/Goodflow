 ...
 */
  ...
  final class Tpl {
    ...
    public static function setConfigure($key, $value = null) {
      if (!is_null($key)) { // pokud neni key null
     ...
     *
     * @return pocet mazanych prebytku
     */
    public static function synchronizeCron() {
      ...
      if (file_exists($map)) {
        ...
        $file_system = Core::getListFile(array('path' => $compile_dir, 'filter-' => array('map')));
...
//TODO removeAll (smazani vsech souboru)
    /**
  ...
  //~ var_dump($codeSplit);
      ...
      if ($codeSplit) {
        ...
        foreach ($codeSplit as $html) {
          ...
          // zpracovani else podminky a else cyklu
