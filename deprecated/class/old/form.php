 ...
 */
  ...
  final class Form {
    const VERSION = 2.56;
//FIXME zabudovat CSRF attack. proti nekolikanasobnemu odeslani formulare
...
//TODO mit moznost aplikovat na vystupy fukci trim! - nejak volitelne!?!
    ...
    //public function setReturnValues() {}
...
//FIXME ma tehdenci cyklit a tobrazovat nektere komponenty podle poctu volani v ramci instance!
//zamezeni je tak ze se dotycny vystup bude  volat je 1x!!!
...
//$lastitm = NULL;
//~ if ($lastitm != $itm->getId()) {
//~ $row[] = $itm;
//~ }
//var_dump('a', $itm->getId(), $lastitm);
//$lastitm = $itm->getId();
            } else {
              //TODO externi nastavovani: class, id, apod.
              $itm = Html::elem('div')->insert(Html::elem('span')->setText($group['name']))
                                        ->insert($r);
              $row[] = $itm;

//~ if ($lastitm != $itm->getId()) {
//~ }
//var_dump('b', $itm->getId(), $lastitm);
//$lastitm = $itm->getId();
            }
...
//var_dump($row);
      $fieldset = Html::elem('fieldset')->insertContent($row);
...
//hloubka
    ...
    private function buildLabel($settings, $element) {
      ...
      if (!empty($label)) {
        ...
        $span[] = Html::elem($span_elem)->setText($label);
      }
      ...
      $result = Html::elem('label')->class(Core::isFill($settings, 'label_class', NULL))
...
//TODO moznost pridavat volitelne parametry jako ze settings:, array('id' => 'back_link_central')
        ...
        $maxval = Core::isFill($settings, 'max', -1);
        $max = ($maxval == -1 ? null : $maxval);  //if max equal -1 then max is infinite
...
//var_dump($name, $multiple_name);
        ...
        switch ($method) {
          case 'addText':
            $element = Html::elem('input')->type('text')
                                          ->class($input_class)
                                          ->name($multiple_name)
                                          ->value($value)
          ...
          case 'addPassword':
            $element = Html::elem('input')->type('password')
                                          ->class($input_class)
                                          ->name($multiple_name)
                                          ->value($value)
                                          ->autocomplete($autocomplete)
          ...
          case 'addTextArea':
            ...
            $element = Html::elem('textarea')->name($multiple_name)
                                              ->class($input_class)
                                              ->cols($cols)
                                              ->rows($rows)
                                              ->placeholder($placeholder)
          ...
          case 'addHidden':
            $element = Html::elem('input')->type('hidden')
                                          ->class($input_class)
                                          ->name($multiple_name)
                                          ->value($value)
                                          ->readonly($readonly)
          ...
          case 'addCheckbox':
            $element = Html::elem('input')->type('checkbox')
                                          ->class($input_class)
                                          ->name($multiple_name)
                                          ->value($value)
                                          ->checked($checked)
          ...
          case 'addCheckGroup':
            ...
            if (!empty($value)) {
              foreach ($value as $key => $val) {
                $row[] = Html::elem('input')->type('checkbox')
                                            ->class($input_class)
                                            ->name($multiple_name)
                                            ->value($key)
                                            ->checked(is_array($checked) ? in_array($key, $checked) : $key == $checked);
                $row[] = Html::span()->setText($val);
              }
            }
          ...
          case 'addSelect':
            ...
            if (!empty($value)) {
              foreach ($value as $key => $val) {
                if (is_array($val)) {
                  $group = Html::elem('optgroup')->label($key);
                  $r = array();
                  foreach ($val as $k => $v) {
                    $r[] = Html::elem('option')->value((string) $k)
                                                ->selected(is_array($selected) ? in_array($k, $selected) : $k == $selected)
                                                ->setText($v);
                  }
                  $row[] = $group->insert($r);
                } else {
                  $row[] = Html::elem('option')->value((string) $key)
            ...
            $element = Html::elem('select')->name($multiple_name)
                                            ->class($input_class)
                                            ->multiple($multiple)
                                            ->size($size)
                                            ->disabled($disabled)
          ...
          case 'addRadio':
            ...
            if (!empty($value)) {
              foreach ($value as $key => $val) {
                $row[] = Html::elem('span')->setText($val);
                $row[] = Html::elem('input')->type('radio')
                                            ->class($input_class)
                                            ->name($multiple_name)
                                            ->value($key)
          ...
          case 'addImage':
            ...
            $element = Html::elem('input')->type('image')
                                          ->class($input_class)
                                          ->name($name)
                                          ->value($value)
                                          ->src($src)
          ...
          case 'addFile':
            ...
            $element = Html::elem('input')->type('file')
                                          ->class($input_class)
                                          ->accept($accept)
                                          ->name($multiple_name)
                                          ->multiple($multiple)
          ...
          case 'addSubmit': //odesilaci tlacitko, ve formulari bude akceptovano jen jedno!
            ...
            $element = Html::elem('input')->type('submit')
                                          ->class($input_class)
                                          ->name($name)
                                          ->value($value)
                                          ->accesskey($accesskey)
          ...
          case 'addButton':
            $element = Html::elem('input')->type('button')
                                          ->name($name)
                                          ->value($value)
                                          ->accesskey($accesskey)
                                          ->tabindex($tabindex)
          ...
          case 'addReset':
            $element = Html::elem('input')->type('reset')
                                          ->name($name)
                                          ->value($value)
                                          ->accesskey($accesskey)
                                          ->tabindex($tabindex)
          ...
          //html5
          case 'addEmail':
            $element = Html::elem('input')->type('email')
                                          ->name($name)
                                          ->value($value)
                                          ->required($required)
                                          ->accesskey($accesskey)
          ...
          case 'addUrl':
            $element = Html::elem('input')->type('url')
                                          ->name($name)
                                          ->value($value)
                                          ->required($required)
                                          ->accesskey($accesskey)
          ...
          case 'addNumber':
            $element = Html::elem('input')->type('number')
                                          ->name($name)
                                          ->value($value)
                                          ->min($min)
                                          ->max($max)
          ...
          case 'addRange':
            $element = Html::elem('input')->type('range')
                                          ->name($name)
                                          ->value($value)
                                          ->min($min)
                                          ->max($max)
          ...
          case 'addSearch':
            $element = Html::elem('input')->type('search')
                                          ->name($name)
                                          ->value($value)
                                          ->required($required)
                                          ->accesskey($accesskey)
          ...
          case 'addColor':
            $element = Html::elem('input')->type('color')
                                          ->name($name)
                                          ->value($value)
                                          ->required($required)
                                          ->accesskey($accesskey)
          ...
          case 'addDate':
            $element = Html::elem('input')->type('date')
                                          ->name($name)
                                          ->value($value)
                                          ->required($required)
                                          ->accesskey($accesskey)
          ...
          case 'addMonth':
            $element = Html::elem('input')->type('month')
                                          ->name($name)
                                          ->value($value)
                                          ->required($required)
                                          ->accesskey($accesskey)
          ...
          case 'addWeek':
            $element = Html::elem('input')->type('week')
                                          ->name($name)
                                          ->value($value)
                                          ->required($required)
                                          ->accesskey($accesskey)
          ...
          case 'addTime':
            $element = Html::elem('input')->type('time')
                                          ->name($name)
                                          ->value($value)
                                          ->required($required)
                                          ->accesskey($accesskey)
          ...
          case 'addDatetime':
            $element = Html::elem('input')->type('datetime')
                                          ->name($name)
                                          ->value($value)
                                          ->required($required)
                                          ->accesskey($accesskey)
          ...
          case 'addDatetimeLocal':
            $element = Html::elem('input')->type('datetime-local')
                                          ->name($name)
                                          ->value($value)
                                          ->required($required)
                                          ->accesskey($accesskey)
...
//vytvoreni skupiny
    public function addGroup($name, $settings = array()) {
...
//TODO metodu na manualni ukonceni skupiny!!!!

//pridavani backlinku
    public function addBackLink($text, $adress, $args = array(), $settings = array()) {
      $this->element->before[] = Html::elem('a', $settings)->href($adress, $args)
                                                            ->title($text)
                                                            ->setText($text);
      return $this;
    }
...
//vraceni chyb
    public function getErrors($sablona = NULL) {
      ...
      if (!empty($this->element->error)) {
        ...
        if (empty($sablona)) {
          $result = implode(Html::elem('br'), $err);
        } else {
