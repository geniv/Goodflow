 ...
 */
  ...
  class StaticWeb {
    ...
    private static $get_adress = array('action', 'co'); //FIXME uplne jinak!!!!!!!!!!!!
 ...
 */
    public static function getMenu($sablona = NULL, $separate = '') {
      ...
      $current = self::getCurrentAdress();
//TODO pokud je prazdne URL ve tride stranek je chyba! ze je jen posledni!
      $row = array();
      $poc = 0;
      $count = count($pages) - 1;
      foreach ($pages as $url => $class) {
        ...
        if (empty($sablona)) {  //defaultni tvar
          ...
          //TODO zaimplementovat!!!!
          //var_dump(method_exists($class, 'getUserVariable'), is_array($class::getUserVariable()));
          $data = array('href_link' => $link,
                        ...
                        'poc' => $poc,
                        'count' => $count,
                        ) + (is_array($class::getUserVariable()) ? $class::getUserVariable() : array());//$class::getUserVariable();  //+pridani promennych ze stranky
