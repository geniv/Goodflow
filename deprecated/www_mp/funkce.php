...
//dodelat!! nacitat php moduly podle nazvu slozky!
    ...
    $pohyb = $this->NactiUnikatniObsah($this->unikatni["admin_vyber_include_pohyb"],
                                      (!Empty($selectmodule[$mindex - 1]) ? $this->NactiUnikatniObsah($this->unikatni["admin_vyber_include_pohyb_prev"],
                                                                                                      "{$adresa}&amp;include={$selectmodule[$mindex - 1]}/{$modulesoubor[$mindex - 1]}",
                                                                                                      $selectmodule[$mindex - 1],
                                                                                                      $modulesoubor[$mindex - 1]) : ""),
 ...
 */
  private function VypisAdminMountModule()
  {
    ...
    {
      foreach ($res as $data)
      {//var_dump(file_exists($data->include)); //overovat existenci??!
        $result .= $this->NactiUnikatniObsah($this->unikatni["admin_vypis_module"],
                                            $data->id,
                                            $data->include,
                                            $data->class,
                                            ($data->admin ? " checked=\"checked\"" : ""),
                                            $data->databaze,
                                            $this->typdatabaze[$data->uloziste],
                                            ($data->aktivni ? " checked=\"checked\"" : ""),
                                            (in_array($data->id, $this->securemodule) ? " disabled=\"disabled\"" : " onclick=\"ChangeActive({$data->id}, this.checked);\""),
                                            ($editmod_permit ? "?{$this->var->get_kam}={$this->var->adresaadminu}&amp;{$this->var->get_idmodul}={$this->idmodul}{$this->idmoumod}&amp;co=editmod&amp;id={$data->id}" : ""),
                                            ($data->pevneporadi < $this->hranicesecure ?  //ostatni moduly maji pevne razeni: $this->hranicesecure
                                            ...
                                            ($data->uloziste > 0 && $data->pevneporadi >= $this->hranicesecure ?
    ...
    if (!Empty($co))
    {
      switch ($co)
      {
        case "clearerrpag":  //promazani error page
          ...
          $this->AdminAddActionLog("", array(__LINE__, __METHOD__));
          $this->AutoClick(5, "?{$this->var->get_kam}={$this->var->adresaadminu}&{$this->var->get_idmodul}={$this->idmodul}{$this->idcontrol}");  //auto kliknuti
        break;
        ...
        case "clearerrlog":  //promazavani error logu
          ...
          $this->AdminAddActionLog("", array(__LINE__, __METHOD__));
          $this->AutoClick(1, "?{$this->var->get_kam}={$this->var->adresaadminu}&{$this->var->get_idmodul}={$this->idmodul}{$this->idcontrol}");  //auto kliknuti
        break;
