  ...
  //type: main/sub
  function GetUpdate(classa, type, ret, result)
  {
    $.post("ajax_funkce.php",
      "action=getupdate&class="+classa+"&type="+type,
        function(theResponse)
        {
          $(ret).html(theResponse);
          ...
          {
            datum = new Date();
            $(result).html((datum.getHours() < 10 ? '0' : '')+datum.getHours()+':'+(datum.getMinutes() < 10 ? '0' : '')+datum.getMinutes()+':'+(datum.getSeconds() < 10 ? '0' : '')+datum.getSeconds());
          }
        }
      );
