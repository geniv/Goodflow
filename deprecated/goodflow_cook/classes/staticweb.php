 ...
 */
  ...
  class StaticWeb {
     ...
     *
     * @param configure pole konfigurace, title - text vkaldany pred jmeno, separator - oddelovac, exception - vyjimka pri ktere nezobrazuje nazev sekce
     * @return text pro title
     */
    public function getTitle(array $configure = array()) {
      $conf = array('title' => '', //pred title
                    'separator' => ' - ',
                    'exception' => array());  //vyjimka pri ktere nepripojuje nazev

      $conf = array_merge($conf, $configure);
      $sub = Route::getRequest();
      $result = '';
      if (!in_array($sub, $conf['exception'])) {
        $name = Core::isNull(self::getCurrentData('name'), 'name');
        $result = implode($conf['separator'], array($conf['title'], $name));
      } else {
        $result = $conf['title'];
      }
      return $result;
    ...
    /**
     * rekurzivni pocitani polozek menu, akceptuje skryte stranky
     *
    ...
    private function _getCountMenu($array = null) {
      ...
      foreach ($source as $class) {
        if (is_array($class)) {
          $result += $this->_getCountMenu($class);
        } else {
    ...
    /**
     * lokalni pocitani polozek menu, akceptuje skryte stranky
     *
     * @param source zdrojove pole
     * @return pocet polozek
     */
    private function _getLocalCountMenu($source) {
      $result = count($source) - 1;
      foreach ($source as $class) {
        if (!is_array($class)) {
          $visible = (defined($class.'::VISIBLE') ? $class::VISIBLE : true);
    ...
    private function _getMenu($skeleton, $array = null, $datas = array()) {
      ...
      $datas['count']['local'][$datas['level']] = $this->_getLocalCountMenu($source);
      ...
      foreach ($source as $url => $class) {
        ...
        if (!is_array($class)) {
          ...
          if ($visible) { //osetruje viditelnost bloku menu
            ...
            $curr = $this->web->route->getRequestUri(); // $this->web->url
            $url_merge = array_merge($datas['lasturl'], array($url));
            $_url = implode('/', $url_merge);

            $merge_filter = array_filter($url_merge);
            $slice_merge = array_slice($url_merge, 0, count($merge_filter));
            $slice_url = array_slice($this->web->url, 0, count($merge_filter));

            $np = array('url' => $_url,
                        'active' => ($curr == $_url),
                        'subactive' => (!empty($slice_merge) && !empty($slice_url) && $slice_merge == $slice_url),
                        );
