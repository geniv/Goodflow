...
class StaticStrankovani extends DefaultModule
{
 ...
 *
 * pouziti:
 * list($str, $limit) = $this->var->main[0]->NactiFunkci("StaticStrankovani", "Strankovani", 10, $pocet_radku, "url_adresa", "limit");
 * $strankovani = $this->var->main[0]->NactiFunkci("StaticStrankovani", "Strankovani", array("na_stranku" => 10, "pocet_radku" => $pocet, "baseurl" => "/", "typ" => "limit|array", "tvar" => 1));
 *
 ...
 */
  public function Strankovani($nastaveni)
  {
    $na_stranku = $nastaveni["na_stranku"];
    $pocet_radku = $nastaveni["pocet_radku"];
    $adresa = $nastaveni["baseurl"];
    $typ = (!Empty($nastaveni["typ"]) ? $nastaveni["typ"] : "limit");
    $tvar = (!Empty($nastaveni["tvar"]) ? $nastaveni["tvar"] : 1);
//$na_stranku, $pocet_radku, $adresa = "", $typ = "limit", $tvar = 1
...
//var_dump($pocetstran);
    ...
    $num_od = abs(($strana - 1) * $na_stranku);
    switch ($typ)
    {
      default:
      case "limit":
        $result[1] = "LIMIT {$num_od}, {$na_stranku}"; //dodatecny dotaz do DB
      break;
