...
class DefaultModule
{
  private $adresa_menu, $typdb, $ukazatel, $var, $trida,
          $cesta, $newcesta, $nazvydb, $pripojeno;
  private $kodroz = "0-0";
 ...
 * @param index cislo modulu v poli modulu
 ...
 */
  public function NastavKomunikaci(&$var, $index, $ajax = false)
  {
    $this->var = $var;
    $this->typdb = $this->var->moduly[$index]["uloziste"];  //$uloziste;
    $this->trida = strtolower($this->var->moduly[$index]["class"]);  //jako predpona do databaze pro tabulky, musi byt lower! $class
    $path = dirname($this->var->moduly[$index]["include"]); //path cesty k databazi
    //$this->cesta = "{$path}/{$this->var->moduly[$index]["databaze"]}";  //$cesta;
    $this->cesta = ($ajax ? $this->var->moduly[$index]["databaze"] : "{$path}/{$this->var->moduly[$index]["databaze"]}");
 ...
 */
  public function PripojeniDatabaze(&$error)
  {
    ...
    switch ($this->typdb)
    {
      default:
      case "sqlite":
        $path = dirname($this->cesta);
        $globalpath = dirname($path);

        if (file_exists($this->cesta) &&  //stryktni osetreni zapisovatelnosti
            is_writable($this->cesta) &&
            is_writable($path) &&
            is_writable($globalpath))
        { //idealni stav
        ...
        {
          if (!file_exists($this->cesta)) //pokud neexistuje, vytvori
          {
            if (!$this->ukazatel = @new SQLiteDatabase($this->cesta, 0777, $error))
            {
              $error = "Nepovedlo se vytvořit SQLite databazi: {$this->cesta}";
            }
          }

          if (!is_writable($this->cesta)) //kdyz nelze zapisovat
          {
            $error = "Nelze zapisovat data do SQLite databaze: {$this->cesta}";
          }

          if (!is_writable($path))
          {
            $error = "Nelze zapisovat data do: {$path}";
          }

          if (!is_writable($globalpath))
          {
            $error = "Nelze zapisovat data do: {$globalpath}";
          }
        }
      ...
      case "mysqli":  //mysql_dbname, mysql_host, mysql_user, mysql_pass, mysql_port
        $this->ukazatel = @new mysqli($this->var->main[0]->DekodujText($this->var->mysql_host),
                                      $this->var->main[0]->DekodujText($this->var->mysql_user),
                                      $this->var->main[0]->DekodujText($this->var->mysql_pass),
                                      $this->var->main[0]->DekodujText($this->var->mysql_dbname),
                                      $this->var->mysql_port);
 ...
 * @return puvodni text
 */
  public function BackChangeChar($text, $komplexne = true)
  {
    $result = "";
    switch ($this->typdb)
    {
 ...
 */
  ...
  {
    $path = dirname($this->newcesta); //slozka z cesty
  ...
  public function ExistujeTabulka($tabulka, &$error)
  {
    ...
    switch ($this->typdb)
    {
      ...
      case "mysqli":
        ...
        if ($res = @$this->ukazatel->query("SHOW TABLES WHERE `Tables_in_{$dbname}`='{$tabulka}';"))
        {
 ...
 */
  public function lastInsertRowid()
  {
    ...
    switch ($this->typdb)
    {
      default:
      case "sqlite":
        $result = $this->ukazatel->lastInsertRowid();
      break;

      case "mysqli":
        $result = $this->ukazatel->insert_id;
      break;
    }
