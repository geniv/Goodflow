 ...
 * @author     Jakub Vrana
 */
class NTableSelection extends NObject implements Iterator, ArrayAccess, Countable // not IteratorAggregate because $this->data can be changed during iteration
{
	/** @var NConnection */
	public $connection;
	...
	/** @var array of [primary key => NTableRow] readed from database */
	protected $rows;

	/** @var array of [primary key => NTableRow] modifiable */
	protected $data;
	...
	/** @var array of referenced NTableSelection */
	protected $referenced = array();

	/** @var array of [sql => [column => [key => NTableRow]]] used by NGroupedTableSelection */
	protected $referencing = array();

	/** @var array of [sql => [key => NTableRow]] used by NGroupedTableSelection */
	 ...
	 * @param  mixed array($column => $value)|Traversable for single row insert or NTableSelection|string for INSERT ... SELECT
