...
class NRoute extends NObject implements IRouter
{
	...
	private function setMask($mask, array $metadata)
	{
		...
		// PARSE MASK
		$parts = NString::split($mask, '/<([^>#= ]+)(=[^># ]*)? *([^>#]*)(#?[^>\[\]]*)>|(\[!?|\]|\s*\?.*)/'); // <parameter-name[=default] [pattern] [#class]> or [ or ] or ?...
		...
		if (isset($parts[$i - 1]) && substr(ltrim($parts[$i - 1]), 0, 1) === '?') {
			$matches = NString::matchAll($parts[$i - 1], '/(?:([a-zA-Z0-9_.-]+)=)?<([^># ]+) *([^>#]*)(#?[^>]*)>/'); // name=<parameter-name [pattern][#class]>
