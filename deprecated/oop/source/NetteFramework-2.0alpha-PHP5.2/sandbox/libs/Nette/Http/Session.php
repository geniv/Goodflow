...
class NSession extends NObject
{
	...
	public function setExpiration($time)
	{
		if (empty($time)) {
			...
			$time = NTools::createDateTime($time)->format('U') - time();
			return $this->setOptions(array(
				'gc_maxlifetime' => $time,
	...
	private function sendCookie()
	{
		...
		$this->getHttpResponse()->setCookie('nette-browser', $_SESSION['__NF']['B'], NHttpResponse::BROWSER, $cookie['path'], $cookie['domain']);
	}
