...
/**
 * NMail provides functionality to compose and send both text and MIME-compliant multipart email messages.
 *
 * @author     David Grudl
 *
...
class NMail extends NMailMimePart
{
	...
	public function addEmbeddedFile($file, $content = NULL, $contentType = NULL)
	{
		...
		return $this->inlines[$file] = $this->createAttachment($file, $content, $contentType, 'inline')
			->setHeader('Content-ID', '<' . NString::random() . "@$hostname>");
	}
	...
	private function createAttachment($file, $content, $contentType, $disposition)
	{
		...
		$part->setContentType($contentType ? $contentType : NTools::detectMimeTypeFromString($content));
	...
	protected function build()
	{
		$mail = clone $this;
		$hostname = isset($_SERVER['HTTP_HOST']) ? $_SERVER['HTTP_HOST'] : (isset($_SERVER['SERVER_NAME']) ? $_SERVER['SERVER_NAME'] : 'localhost');
		$mail->setHeader('Message-ID', '<' . NString::random() . "@$hostname>");
