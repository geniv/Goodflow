	...
	private function parse($s)
	{
		...
		foreach ($this->tags as $tag) {
			if (!$tag->isMacro && !empty($tag->attrs)) {
				throw new NLatteException("Missing end tag </$tag->name> for macro-attribute " . self::HTML_PREFIX . implode(' and ' . self::HTML_PREFIX, array_keys($tag->attrs)) . ".", 0, $this->line);
			}
		}
	...
	private function contextText()
	{
		...
		if (!$matches || !empty($matches['macro']) || !empty($matches['comment'])) { // EOF or {macro}
			...
			do {
				...
				if (!$tag) {
					//throw new NLatteException("End tag for element '$matches[tag]' which is not open.", 0, $this->line);
					$tag = (object) NULL;
					$tag->name = $matches['tag'];
					$tag->isMacro = NString::startsWith($tag->name, self::HTML_PREFIX);
				}
		...
		if (!$matches || !empty($matches['macro']) || !empty($matches['comment'])) { // EOF or {macro}
			...
			if ($tag->isMacro || !empty($tag->attrs)) {
				if ($tag->isMacro) {
					...
					if ($code === FALSE) {
						throw new NLatteException("Unknown macro-attribute " . self::HTML_PREFIX . implode(' or ' . self::HTML_PREFIX, array_keys($tag->attrs)), 0, $this->line);
					}
					if ($isEmpty) {
