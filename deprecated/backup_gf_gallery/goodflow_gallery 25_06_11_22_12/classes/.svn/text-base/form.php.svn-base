...
//FIXME osetrit tady kdyz budou parametry jinak a nebo nebudou zadane!!!!!
      ...
      $selected = Core::isFill($settings, 'selected', NULL);
...
//TODO nema u byt jeste pripadne nejaky obal nebo nejaky span element
        ...
        case 'addSelect':
          ...
          foreach ($value as $key => $value) {
            $row[] = self::elem('option')->value($key)->selected($key == $selected)->setText($value);
          }

          $element = self::elem('select')
                          ->name($multiple_name)
                          ->multiple($multiple)
                          ->insert($row)
                          ;
...
//http://diveintohtml5.org/forms.html
    ...
    //public function addMultiSelect() {} s optgroup
