 ...
 */
  ...
  final class Core {
     ...
     *
     * @param query array url
     * @param settings array settings, rewrite(boolean), amp(string)
     * @return adress web page
     */
    public static function getAbsoluteUrl(array $settings = array()) {
      $script = explode('/', $_SERVER['SCRIPT_NAME']);
      $absolute_path = explode('/', self::getAbsoluteWebPath());
...
//absolutni path webu
    public static function getAbsoluteWebPath() {
      $script = explode('/', $_SERVER['SCRIPT_FILENAME']);  //rozdeleni script pathe
      $dir = explode('/', __DIR__); //rozdeleni aktualniho pathe
...
//vytvoreni linku jako ve Form()->href()
    public static function makeUrl($url, $param) {
      $result = $url;
      if (!empty($param)) {
        $result = sprintf('%s?%s', $url, http_build_query($param, NULL, '&amp;'));
      }
