...
<td>$mini</td>
<td>$fot</td>
<td><a href=\"index.php?kam=$kam&galerie=$galerie&cislo=$i\" class=\"odkaz\">Smazat</a></td>
</tr>";
}//end for
print
"</table>";
...
//ftp_delete($conn_id,"../".ExtrahujNazevZOdkazu($foto[($cislo*$del1)]));//smazat soubor
  ...
  // zjistim si cislo adresare galerie
  $file ="obsah_galerii_alcknasdjcnisdjnniudasbvoesdjvsojhnviwruohnboiweunbfvweoiv.php";
  $fp   = FOpen($file,"r");
  $data = Explode("--GAL--",FRead($fp,DelkaOteviraniSouboru(".")));
  FClose($fp);
 
  // zahodim obrazek 
  unlink("../".$data[$del1*$galerie+3]."/".$foto[2]);
  
  // zahodim miniauturu
  unlink("../".$data[$del1*$galerie+3]."/miniatury/".$foto[1]);

//}
/*
//var_dump($foto[1],$foto[2],$galerie);
//die("here");
$soubor="foto_{$galerie}_qowiifhnisvjnsofvnisfcnoadihcnidfjnvoiadhviosfjfiweuhnvjsoiddfhnisdvjodfhviujdffvisujhcvqiecnviedjwfnvijwednviuqedfbv.php";
$u=fopen($soubor,"r");
$udaj=explode("--FOT--",fread($u,DelkaOteviraniSouboru(".")));
