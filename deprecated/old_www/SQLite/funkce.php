<?php
	...
	//****************************************************************************
		...
		{	
			if (filesize($nazev) == 0)
			{
				if ($objekt->query("CREATE TABLE deklarace (id INTEGER AUTO_INCREMENT PRIMARY KEY , nazev VARCHAR(300))"))
				{
	...
	//****************************************************************************
		function Menu()
		{
			if (!Empty($_POST["login"]) && !Empty($_POST["heslo"]))
			{
				...
				{
					SetCookie("CSSJMENO", md5($_POST["login"]), Time() + 31536000); //zápis do cookie
      		SetCookie("CSSHESLO", md5($_POST["heslo"]), Time() + 31536000);
			...
			if ($res = $objekt->query("SELECT deklarace.nazev as deklarace, hodnota.nazev as hodnota, hodnota.poznamka as poznamka FROM deklarace, hodnota WHERE deklarace.id=hodnota.iddek ORDER BY deklarace.id"))
			{
				...
				if ($poc > 0)
				{
					...
					for ($i = 0; $i < $poc; $i++)
					{
						...
						if ($data->poznamka != "")
						{
							$pozn = "/* ".htmlspecialchars_decode($data->poznamka, ENT_QUOTES)." */";	//velmi důležité kodování!!
						}
							else
						{
							$pozn = "";
						...
						{
							$vystup .= "\t".htmlspecialchars_decode($data->hodnota, ENT_QUOTES).";\t\t{$pozn}{$kon}";
						}
							else
						{
							if ($i > 1)	//dosazování }
							...
							$vystup .= "{$zak}".htmlspecialchars_decode($data->deklarace, ENT_QUOTES)." {{$kon}\t".htmlspecialchars_decode($data->hodnota, ENT_QUOTES).";\t\t{$pozn}{$kon}";
						}
				...
				if ($poc > 0)
				{
					...
					for ($i = 0; $i < $poc; $i++)
					{
						...
						$vypis .= "<tr>
												<td>$data->id</td>
												<td>$data->nazev</td>
												<td><a href=\"?action=edit_dek{$this->oddel}{$data->id}\">uprav</a></td>
											</tr>";
					}
					$vypis .= "</table>";
				}
				...
				if ($poc > 0)
				{
					...
					for ($i = 0; $i < $poc; $i++)
					{
						...
						$vypis .= "<tr>
												...
												<td>$data->hodnota</td>
												<td>$data->poznamka</td>
												<td><a href=\"?action=edit_hod{$this->oddel}{$data->id}\">uprav</a></td>
											</tr>";
					}
					$vypis .= "</table>";
				}
				...
				if ($poc > 0)
				{
					...
					for ($i = 0; $i < $poc; $i++)
					{
						...
						$vypis .= "<tr>
												<td>$data->id</td>
												<td>$data->nazev</td>
												<td><a href=\"?action=del_dek{$this->oddel}{$data->id}\">smazat</a></td>
											</tr>";
					}
					$vypis .= "</table>";
				}
				...
				if ($poc > 0)
				{
					...
					for ($i = 0; $i < $poc; $i++)
					{
						...
						$vypis .= "<tr>
												...
												<td>$data->hodnota</td>
												<td>$data->poznamka</td>
												<td><a href=\"?action=del_hod{$this->oddel}{$data->id}\">smazat</a></td>
											</tr>";
					}
					$vypis .= "</table>";
				}
	...
	//****************************************************************************
		...
		{
			$nazev = stripslashes(htmlspecialchars($nazev, ENT_QUOTES));
			$poznamka = stripslashes(htmlspecialchars($poznamka, ENT_QUOTES));
