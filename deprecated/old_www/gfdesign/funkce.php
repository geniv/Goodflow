 ...
 */
  public function AbsoluteUrl()
  {
    ...
    $result = "http://{$_SERVER["SERVER_NAME"]}{$path}/";
  ...
  private function KontrolaAktualizace()
  {
    if ($this->var->aktualizovat)
    {
      ...
      for ($i = 0; $i < count($moduly); $i++)
      {
        ...
        if (!Empty($zprava_velikost))
        {
            ...
            else
          {
              ...
              else
            {
              ...
              {
                $info = "novější datum na depoztu - nesedí velikost <a href=\"{$this->var->depozitar}?{$this->get_download}={$cesta}&amp;{$this->get_zdrojstazeni}={$_SERVER["SERVER_NAME"]}\" title=\"sosnout\">sosnout</a> <a href=\"?{$this->var->get_kam}={$this->var->adresaadminu}&amp;{$this->var->get_idmodul}={$this->idmodul}{$this->idupp}&amp;file={$cesta}\" title=\"upnout\">upnout</a>";
              }
            }
          }
        }
        ...
        $result .=
        "<p>
          {$nazev} ([{$adresar}] - {$soubor}) -<br /><strong>{$datum} ({$this->Velikost($velikost)})</strong> || depozit: <strong>{$zprava_datum} ".(!Empty($zprava_velikost) ? "({$this->Velikost($zprava_velikost)})" : "")."</strong> -<br />{$info}
          <br />
          <br />
        </p>";
      }
 ...
 *
 * pouziti: <strong>$email = $this->var->main[0]->NactiFunkci("Fukce", "KontrolaEmailu", "email@email.cz");</strong>
 *
 ...
 * Detekce linuxu
 *
 * pouziti: <strong>".($this->DetekceLinuxu() ? "je to linux" : "je to mrkvosoft")."</strong>
 *
 ...
 * Prepocet velikosti
 *
 * pouziti: <strong>$velikost = $this->var->main[0]->NactiFunkci("Funkce", "Velikost", file_size($soubor));</strong>
 *
...
/**
 *
 * Prepisuje obycejnou adresu pro htaccess format
 *
...
/**
 *
 * Prepisuje zpet textovou adresu do neupravene
 *
 * @param adresa vstupni adresa
