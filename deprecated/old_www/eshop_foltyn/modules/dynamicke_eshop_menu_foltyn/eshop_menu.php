    ...
    if ($res = @$this->sqlite->query("SELECT id, nazev, submenu, zanoreni FROM menu WHERE zanoreni=0 ORDER BY LOWER(nazev) ASC;", NULL, $error))
    {
      if ($res->numRows() != 0)
      {
        while ($data = $res->fetchObject())
        {
          ...
          $result .=  //  - ({$data->id}, {$data->zanoreni})
          "<a href=\"?{$this->var->get_kam}={$data->id}\" title=\"{$data->nazev}\">".($oznpodm ? ($subozna ? "" : "") : "")."{$data->nazev}".($oznpodm ? ($subozna ? "" : "") : "")."</a>
 ...
 */
  ...
  {
    $result =
    "<a href=\"?{$this->var->get_kam}={$this->var->adresaadminu}&amp;{$this->var->get_idmodul}={$this->idmodul}\" title=\"\">Úprava menu</a>";
      ...
      if ($res = @$this->sqlite->query("SELECT id, nazev, zanoreni FROM menu WHERE id IN ({$subwhere}) ORDER BY LOWER(nazev) ASC;", NULL, $error))
      {
        if ($res->numRows() != 0)
        {
          while ($data = $res->fetchObject())
          {
            ...
            $result .=                                                                                                                                                                                  //  - ({$data->id}, {$data->zanoreni})
            "<a href=\"?{$this->var->get_kam}={$rozklad}{$data->id}\" class=\"submenu\" title=\"{$data->nazev}\">{$odsazeni}".($oznpodm ? ($subozna ? "" : "") : "")."{$data->nazev}".($oznpodm ? ($subozna ? "" : "") : "")."</a>
 ...
 */
  ...
  {
    $result =
    "<div class=\"soupis_obj\"><strong>Úprava menu</strong>
    <br /><br />
    <a href=\"?{$this->var->get_kam}={$this->var->adresaadminu}&amp;{$this->var->get_idmodul}={$this->idmodul}&amp;co=add&amp;zanoreni=0\" title=\"\">Přidat sekci</a><br />
    <br /><table>
    {$this->AdminVypisMenu()}
    </table>
    </div>";
    ...
    if (!Empty($co))
    {
      switch ($co)
      {
        case "add": //pridavani
          ...
          <form method=\"post\">
            <fieldset>
              <input type=\"input\" name=\"nazev\" /> <br /><br />
              <input type=\"submit\" name=\"tlacitko\" value=\"Přidat sekci\" /><br />
            </fieldset>
          </form>
          ";
        ...
        case "edit":  //editace
          ...
          if ($res = @$this->sqlite->query("SELECT nazev FROM menu WHERE id={$id};", NULL, $error))
          {
            if ($res->numRows() == 1)
            {
              ...
              <form method=\"post\">
                <fieldset>
                  <input type=\"input\" name=\"nazev\" value=\"{$data->nazev}\" /> <br /><br />
    ...
    if ($res = @$this->sqlite->query("SELECT id, nazev, submenu, zanoreni FROM menu WHERE zanoreni=0 ORDER BY LOWER(nazev) ASC;", NULL, $error))
    {
      if ($res->numRows() != 0)
      {
        while ($data = $res->fetchObject())
        {
          ...
          $result .=
          "<tr><td>{$odsazeni} <strong>{$data->nazev}</strong></td>
          <td>{$exter_add}</td>
          <td><a href=\"?{$this->var->get_kam}={$this->var->adresaadminu}&amp;{$this->var->get_idmodul}={$this->idmodul}&amp;co=add&amp;id={$data->id}&amp;zanoreni=".($data->zanoreni + 1)."\" title=\"\">přidat podsekci</a></td>
          <td><a href=\"?{$this->var->get_kam}={$this->var->adresaadminu}&amp;{$this->var->get_idmodul}={$this->idmodul}&amp;co=edit&amp;id={$data->id}\" title=\"\">upravit sekci</a></td>
          <td><a href=\"?{$this->var->get_kam}={$this->var->adresaadminu}&amp;{$this->var->get_idmodul}={$this->idmodul}&amp;co=del&amp;id={$data->id}\" title=\"\" onclick=\"return confirm('Opravdu smazat: \'{$data->nazev}\' ?');\">smazat sekci</a></td></tr>
          {$submenu}";
        }
      }
    }
