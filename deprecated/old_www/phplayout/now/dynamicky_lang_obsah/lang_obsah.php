 ...
 */
  public function LangObsah($adr = NULL, $kontrola = true)
  {
    if (!Empty($adr))
    {
      $adresa = $adr;
    ...
    if ($res = @$this->sqlite->query("SELECT text FROM dynamicky_lang_obsah WHERE adresa='{$adresa}' AND jazyk={$jazyk};", NULL, $error))
    {
      if ($res->numRows() == 1)
      {
        $result = ($kontrola ? htmlspecialchars_decode(html_entity_decode($res->fetchObject()->text, ENT_QUOTES)) : $res->fetchObject()->text);
      }
        else
      {
        if ($this->vypis_chybu)
 ...
 */
  private function VypisSkupiny($skupina)
  {
    ...
    if ($res = @$this->sqlite->query("SELECT id, adresa, text FROM dynamicky_lang_obsah WHERE skupina={$skupina};", NULL, $error))
    {
      if ($res->numRows() != 0)
      {
        while($data = $res->fetchObject())
