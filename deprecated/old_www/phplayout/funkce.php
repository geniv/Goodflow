...
{
  private $start, $konec, $var, $logoff, $unikatni, $absolutni_url;
 ...
 */
  public function __construct(&$var, $index) //konstruktor
  {
    ...
    if ($_GET[$this->var->get_kam] == $this->var->adresaadminu) //includovani jen kdyz je v adminu
    {
 ...
 */
  private function StartSession() //aktvuje session promenne
  {
    ...
    session_register("ADMIN:{$absolutni_url}");
 ...
 */
  ...
  */
        ...
        $result .= ($admin_menu[$i]["zobrazit"] ? $this->NactiUnikatniObsah($this->unikatni["tvar_admin_menu"], // + menu_tvar_logoff
                                                ...
                                                : ($this->var->debug_mod ? $this->NactiUnikatniObsah($this->unikatni["tvar_admin_menu_skryte"],
                                                                                                    "?{$this->var->get_kam}={$this->var->adresaadminu}{$main_href}",
                                                                                                    $admin_menu[$i]["odkaz"],
                                                                                                    $id,
                                                                                                    $class,
 ...
 */
  ...
  {
    $endtime = mktime(date("H", $_SERVER["REQUEST_TIME"]) + $this->var->admin_expire[0],
                      date("i", $_SERVER["REQUEST_TIME"]) + $this->var->admin_expire[1],
                      date("s", $_SERVER["REQUEST_TIME"]) + $this->var->admin_expire[2],
                      date("n", $_SERVER["REQUEST_TIME"]), date("j", $_SERVER["REQUEST_TIME"]), date("Y", $_SERVER["REQUEST_TIME"]));

    $result = $this->NactiUnikatniObsah($this->unikatni["text_cas_aktualizace"],
                                        date("H:i:s", $_SERVER["REQUEST_TIME"]),
                                        $this->var->admin_expire[0],
                                        $this->var->admin_expire[1],
                                        $this->var->admin_expire[2],
                                        date("H:i:s", $endtime));
 ...
 */
  private function Prihlasovani()
  {
    ...
    {
      unset($_SESSION["ADMIN:{$absolutni_url}"]["USER"]); //zrusi jen login a heslo
      unset($_SESSION["ADMIN:{$absolutni_url}"]["PASS"]);
        ...
        $this->AkceptujProhlizece())  //checkuje prohlizec
    {
        ...
        else
      {
        if ($this->Autorizace($login, $heslo))  //kdyz je prihaseno
        {
          ...
          $_SESSION["ADMIN:{$absolutni_url}"]["USER"] = md5(md5($login));
          $_SESSION["ADMIN:{$absolutni_url}"]["PASS"] = md5(md5($heslo));

          if (!Empty($_POST["log_ad"]))  //uklad po sobe $_POST
          {
            $autourl = (!Empty($_SESSION["ADMIN:{$absolutni_url}"]["LASTURL"]) ? $_SESSION["ADMIN:{$absolutni_url}"]["LASTURL"] : "{$this->var->get_kam}={$this->var->adresaadminu}");
            $this->AutoClick(0, "{$absolutni_url}?{$autourl}");
          }

          $_SESSION["ADMIN:{$absolutni_url}"]["TIME"] = $nextdate;
        }
      }
    }
      else
    {
      if ($_GET[$this->var->get_kam] == $this->var->adresaadminu &&
          $this->Autorizace($_SESSION["ADMIN:{$absolutni_url}"]["USER"], $_SESSION["ADMIN:{$absolutni_url}"]["PASS"], false) &&
          $this->AkceptujProhlizece())  //pro admin, checkuje prohlizec
      {
        $this->var->aktivniadmin = true;
        $_SESSION["ADMIN:{$absolutni_url}"]["LASTURL"] = $_SERVER["QUERY_STRING"];  //pro zpetne vraceni na dane misto

        if ($_SERVER["REQUEST_TIME"] > $_SESSION["ADMIN:{$absolutni_url}"]["TIME"])  //expire
        {
          unset($_SESSION["ADMIN:{$absolutni_url}"]["USER"]); //zrusi jen login a heslo
          unset($_SESSION["ADMIN:{$absolutni_url}"]["PASS"]);
          unset($this->var->backadmin);

          $this->AutoClick(0, "{$absolutni_url}?{$this->var->get_kam}=logoff"); //kdyz vyprsi cas expirace tak autoclick
        }
        ...
        else
      {
        ...
        $this->var->backadmin = $_SESSION["ADMIN:{$absolutni_url}"]["USER"]; //pro zobrazeni odkazu na indexu

        if ($_SERVER["REQUEST_TIME"] > $_SESSION["ADMIN:{$absolutni_url}"]["TIME"] &&  //presmerovani pri vyprseni
            $_GET[$this->var->get_kam] == $this->var->adresaadminu)
        {
          $_SESSION["ADMIN:{$absolutni_url}"]["LASTURL"] = $_SERVER["QUERY_STRING"];
          $this->AutoClick(0, $absolutni_url);
        }
      }
    }
 ...
 */
  ...
  {
    $result = ($this->var->adminpristup[($kodovat ? md5(md5($login)) : $login)] == ($kodovat ? md5(md5($heslo)) : $heslo) ? true : false);
 ...
 */
  private function GenerovaniErrorPage()
  {
        ...
        $_GET[$this->var->get_kam] == $this->var->adresaadminu) //pokud je v adminu
    {
      ...
      {
        $pole = $error[$i];
        $pole["absolutni_url"] = $absolutni_url;  //prida absolutni cestu
