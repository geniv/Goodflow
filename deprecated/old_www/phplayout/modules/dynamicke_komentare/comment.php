...
/**
 *
 * Blok dynamickych komentaru
 *
 * public funkce:\n
 * construct: DynamicObsah - hlavni konstruktor tridy\n
 * Komentare() - hlavni vypis obsahu\n
 * AdminObsah() - obsah adminu\n
 *
 */

class DynamicComment extends DefaultModule
{
  private $var, $dbname, $dirpath, $absolutni_url, $unikatni, $dbpredpona;
  public $idmodul = "dynocomm";  //id pro rozliseni modulu v adminu
 ...
 */
  ...
  {
    if (!$this->InstalaceDatabaze("CREATE TABLE {$this->dbpredpona}komentare_skupina (
 ...
 */
  public function Komentare($adr = NULL, $kontrola = true, $pridavek = NULL)
  {
    if (!Empty($adr))
    {
      $adresa = $adr;
                            ...
                            WHERE adresa='{$adresa}';", $error))
    {
      if ($this->numRows($res) == 1)
      {
        ...
        //pretny prevod kvuli ampersantum a tomu ze sqlite preklada texty uplne
        ...
        $result = $this->PrevodUnikatnihoTextu(($kontrola ? $special : $text), $doplnek);
      }
        else
      {
        if ($this->vypis_chybu)
    ...
    if ($res = $this->query("SELECT id, nazev, popisek FROM {$this->dbpredpona}gtk_skupina WHERE id={$id};", $error))
    {
      if ($this->numRows($res) == 1)
      {
        ...
        $result = $this->NactiUnikatniObsah($this->unikatni["admin_vypis_obsahu_skupiny"],
                                            $data->nazev,
                                            htmlspecialchars_decode(html_entity_decode($data->popisek)),
        ...
        while($data = $this->fetchObject($res))
        {
          $result .= $this->NactiUnikatniObsah($this->unikatni["admin_vypis_edit_link_skupina{$doplnek}"],
                                              ...
                                              ($this->povolit_pridani ? $this->NactiUnikatniObsah($this->unikatni["admin_skupina_del_link{$doplnek}"],
                                                                                                  ...
                                                                                                  $data->adresa) : ""),
                                              $data->adresa,
                                              htmlspecialchars_decode(html_entity_decode($data->text)));
        }
      }
    }
      else
