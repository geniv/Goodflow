 ...
 * PocetPolozekObsahu() - funkce pro navraceni poctu polozek podle dane adresy\n
...
class DynamicObsahEshop extends DefaultModule
{
  private $var;
  private $sqlite;
  private $dbname;// = ".dbdynobsaheshop.sqlite2";
  public $idmodul = "dynobsaheshop";  //id pro rozliseni modulu v adminu
  ...
  private $strankovani = 3; //pocet radku strankovani
 ...
 */
  public function __construct(&$var, $index) //konstruktor
  {
    ...
    $adresa_menu = array( array("main_href" => "{$this->idmodul}",
                                "odkaz" => "administrace dynamickeho eshop obsahu",
                                "title" => "administrace dynamickeho eshop obsahu",
                                "id" => "",
                                "class" => "",
                                "akce" => ""),
                          );
...
/**
 *
 * Hledani indexu v databazi
 *
 * @param indexy indexy ktere se maji vyhledat
 * @return soupis prislusnych id
 */
  private function HledejAdresyMenu($indexy)
  {
    $dotaz = "";
    $result = "";
    for ($i = 0; $i < count($indexy); $i++)
    {
      settype($indexy[$i], "integer");
      $dotaz .= "adresa LIKE ('%-{$indexy[$i]}-%') OR adresa LIKE ('{$indexy[$i]}-%') OR adresa LIKE ('%-{$indexy[$i]}') OR adresa={$indexy[$i]} OR ";
    }
    $dotaz = substr($dotaz, 0, -4);
    ...
    $result = (!Empty($pole) && count($pole) > 0 ? implode(", ", $pole) : false);
 ...
 */
  ...
  {
    //var_dump($this->var->main[0]->NactiFunkci("DynamicEshopMenu", "PrevodTextoveAdresy"));

    //$adresa = (!Empty($_GET[$this->var->get_kam]) ? $this->ValiditaAdresy($_GET[$this->var->get_kam]) : $this->PrvniPolozka());

    $subadresa = (!Empty($_GET[$this->var->get_kam]) ?
                  ($this->var->htaccess ? $this->var->main[0]->NactiFunkci("DynamicEshopMenu", "PrevodTextoveAdresy") :
                  explode("-", $_GET[$this->var->get_kam])) : $this->PrvniPolozka());
//var_dump($subadresa);
//var_dump($this->var->main[0]->NactiFunkci("DynamicEshopMenu", "RekurzivniMazani", $subadresa[count($subadresa) - 1]));
//var_dump($subadresa[count($subadresa) - 1]);
    //vraci vsechna prislusna zanoreni, vsechyn indexy podsekci
    $vypis = explode("-", "{$this->var->main[0]->NactiFunkci("DynamicEshopMenu", "RekurzivniMazani", $subadresa[count($subadresa) - 1])}{$subadresa[count($subadresa) - 1]}");
//var_dump($vypis);
    $adresy = $this->HledejAdresyMenu($vypis);
    $dotaz = "id IN ({$adresy})";
      ...
      else
    {
      ...
      settype($str, "integer");
//var_dump($_GET);
      //$stankovani = $this->Strankovani($str, "?{$this->var->get_kam}={$_GET[$this->var->get_kam]}&amp;sort={$sort}&amp;view={$view}", count(explode(",", $adresy)), $limit);
//var_dump($this->strankovani);
      list($stankovani, $limit) = $this->var->main[0]->NactiFunkci("StatickeStrankovani", "Strankovani", $this->strankovani, count(explode(",", $adresy)), "?{$this->var->get_kam}={$_GET[$this->var->get_kam]}&amp;sort={$sort}&amp;view={$view}", "limit");
//tvar: ..../ahoj-sekce/serazeno-podle:ceny/pohled:obr/strana:1
      $posled = explode("%%%%", $_GET[$this->var->get_kam]);

      $result =  //hlavni vypis polozek, lze radit a prepinat zobrazeni
      "řadit podle:
      ".($sort == "a_cena" ?
      "<a href=\"".($this->var->htaccess ?
                  "{$posled[count($posled) - 1]}/serazeno-podle:ceny/pohled:{$view}/strana:{$str}" :
                  "?{$this->var->get_kam}={$_GET[$this->var->get_kam]}&amp;sort=de_cena&amp;view={$view}&amp;str={$str}")."\">ceny DESC</a>" :
                                      ...
                                      ORDER BY LOWER(adresa) ASC;", NULL, $error))
    {
      if ($res->numRows() != 0)
      {
        while ($data = $res->fetchObject()) //pripadne prepisovac specialni syntaxe a v editoru ajax zobrazovani
        {
          ...
          $nazev = $this->var->main[0]->NactiFunkci("DynamicEshopMenu", "NazevPodleAdresy", $adr[count($adr) - 1]);
 ...
 *
 * pouziti: <strong>$cislo = $this->var->main[0]->NactiFunkci("DynamicObsahEshop", "PocetPolozekObsahu"[, "adresa"]);</strong>
 *
 * @param adr adresa obsahu
 * @return pocet polozek
 */
  public function PocetPolozekObsahu($adr = NULL)
  {
    if (!Empty($adr))
    {
      $adresa = $adr;
