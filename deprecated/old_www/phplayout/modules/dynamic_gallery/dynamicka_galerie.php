 ...
 */
  ...
  {
    if (!$this->InstalaceDatabaze("CREATE TABLE {$this->dbpredpona}gallery (
 ...
 */
  public function Gallery($adr = NULL, $razeni = 0, $tvar = 1)
  {
    ...
    {
      case 1:
        $order = "{$this->dbpredpona}gallery.datum ASC";
      break;

      case 2:
        $order = "{$this->dbpredpona}gallery.datum DESC";
      break;
    ...
    $result = "";
    //vypis galerie
    if ($res = $this->query("SELECT popisek, url FROM {$this->dbpredpona}gallery WHERE adresa='{$adresa}' ORDER BY {$order};", $error))
    {
 ...
 */
  ...
  {
    $result = "";
    if ($res = $this->query("SELECT adresa, count(adresa) pocet FROM {$this->dbpredpona}gallery GROUP BY adresa ORDER BY adresa;", $error))
    {
      if ($this->numRows($res) != 0)
      {
        while ($data = $this->fetchObject($res))
 ...
 */
  private function PocetRadku($adresa, $inc = 0)
  {
    ...
    if (!Empty($adresa))
    {
      if ($res = $this->query("SELECT COUNT(id) pocet FROM {$this->dbpredpona}gallery WHERE adresa='{$adresa}';", $error))
      {
 ...
 */
  private function AdministraceObsahu()
  {
    $result = $this->NactiUnikatniObsah($this->unikatni["admin_obsah"],
                                        ...
                                        "?{$this->var->get_kam}={$this->var->adresaadminu}&amp;{$this->var->get_idmodul}={$this->idmodul}&amp;co=dragndrop",
                                        $this->SeznamAdres(),
    ...
    if (!Empty($co))
    {
      switch ($co)
      {
        ...
        case "dragndrop": //drag'n drop
          $result = $this->NactiUnikatniObsah($this->unikatni["admin_dragndrop_begin"],
                                              $this->absolutni_url,
                                              $this->dirpath,
                                              "?{$this->var->get_kam}={$this->var->adresaadminu}&amp;{$this->var->get_idmodul}={$this->idmodul}");
                                  ...
                                  ORDER BY LOWER(adresa) ASC;", $error))
          {
            if ($this->numRows($res) != 0)
            {
              while ($data = $this->fetchObject($res))
              {
                $result .= $this->NactiUnikatniObsah($this->unikatni["admin_dragndrop_group_begin"],
                                                    $data->adresa);

                if ($res1 = $this->query("SELECT id, adresa, popisek, url, datum, poradi
                                          FROM {$this->dbpredpona}gallery
                                          ...
                                          ORDER BY poradi ASC;", $error1))
                {
                  if ($this->numRows($res1) != 0)
                  {
                    ...
                    {
                      $datum = date($this->NactiUnikatniObsah($this->unikatni["admin_datum_tvar"]), strtotime($data1->datum));
                ...
                $result .= $this->NactiUnikatniObsah($this->unikatni["admin_dragndrop_group_end"],
                                                    $data->adresa);
              }
            }
              else
            {
              $result .= $this->NactiUnikatniObsah($this->unikatni["admin_dragndrop_null"]);
            }
          }
            else
          {
          ...
          $result .= $this->NactiUnikatniObsah($this->unikatni["admin_dragndrop_end"]);
        break;
        ...
        case "add": //pridavani obrazku
          ...
          $datum = date("Y-m-d H:i:s", strtotime($this->ChangeWrongChar($_POST["datum"])));
          ...
          if (!Empty($_POST["tlacitko"]) &&
              ...
              $ob)
          {
            if ($this->queryExec("INSERT INTO {$this->dbpredpona}gallery (id, adresa, popisek, url, datum, poradi, w_mini, h_mini, w_full, h_full) VALUES
        ...
        case "edit":  //editace skupiny
          ...
          if ($res = $this->query("SELECT adresa, popisek, url, datum, poradi, w_mini, h_mini, w_full, h_full FROM {$this->dbpredpona}gallery WHERE id={$id};", $error))
          {
            if ($this->numRows($res) == 1)
            {
              ...
              if (!Empty($_POST["tlacitko"]) &&
                  ...
                  $id != 0)
              {
                if ($this->queryExec("UPDATE {$this->dbpredpona}gallery SET adresa='{$adresa}',
                                                                            popisek='{$popisek}',
                                                                            url='{$url}',
                                                                            datum='{$datum}',
                                                                            poradi={$poradi},
        ...
        case "del": //mazani skupiny
          ...
          if ($res = $this->query("SELECT popisek, url FROM {$this->dbpredpona}gallery WHERE id={$id};", $error))
          {
            if ($this->numRows($res) == 1)
            {
              $data = $this->fetchObject($res);

              if ($this->queryExec("DELETE FROM {$this->dbpredpona}gallery WHERE id={$id};", $error)) //provedeni dotazu
              {
                $navic = $this->SyncFileWithDB(); //synchronizace
 ...
 */
  ...
  {
    if ($res = $this->query("SELECT url FROM {$this->dbpredpona}gallery;", $error))
    {
      if ($this->numRows($res) != 0)
      {
        $i = 0;
            ...
            if ($this->numRows($res1) != 0)
            {
              while ($data1 = $this->fetchObject($res1))
              {
                ...
                $result .= $this->NactiUnikatniObsah($this->unikatni["admin_vypis_obsah"],
                                                    ...
                                                    "{$this->dirpath}/{$this->pathpicture}/{$this->fulldir}/{$data1->url}",
                                                    "{$this->dirpath}/{$this->pathpicture}/{$this->minidir}/{$data1->url}",
                                                    "?{$this->var->get_kam}={$this->var->adresaadminu}&amp;{$this->var->get_idmodul}={$this->idmodul}&amp;co=edit&amp;id={$data1->id}",
                                                    "?{$this->var->get_kam}={$this->var->adresaadminu}&amp;{$this->var->get_idmodul}={$this->idmodul}&amp;co=del&amp;id={$data1->id}");
              }
            }
          }
           else
