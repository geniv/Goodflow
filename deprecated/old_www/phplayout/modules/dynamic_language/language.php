...
class DynamicLanguage extends DefaultModule
{
  ...
  public $idmodul = "language";
  private $ozn_jazyk_l = "[";
  private $ozn_jazyk_r = "]";
  private $cookiename = "LANG";
  private $adrname = "sub";
  private $adrchange = "changelang";
 ...
 */
  public function __construct(&$var, $index) //konstruktor
  {
    ...
    $this->cookiename = $this->NactiUnikatniObsah($this->unikatni["set_cookie_name"]);
    $this->adrname = $this->NactiUnikatniObsah($this->unikatni["set_adrname"]);
    $this->adrchange = $this->NactiUnikatniObsah($this->unikatni["set_adrchange"]);
 ...
 */
  ...
  {
    $result = "";
    if ($res = @$this->sqlite->query("SELECT id, jazyk, zkratka FROM jazyk ORDER BY id ASC;", NULL, $error))
    {
      if ($res->numRows() != 0)
      {
        while ($data = $res->fetchObject())
        {
          ...
          $url = ($this->var->htaccess ? $this->NactiUnikatniObsah($this->unikatni["rewrite_url_seznam_jazyku_{$tvar}"],
                                                                  $this->absolutni_url,
                                                                  $data->zkratka)
                                        : "?{$this->adrname}={$this->adrchange}&amp;id={$data->id}");
        ...
        $_GET[$this->adrname] == $this->adrchange)
    {
      ...
      $this->var->main[0]->AutoClick(0, $this->absolutni_url);  //auto kliknuti
    }
