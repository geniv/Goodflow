 ...
 */
  public function Menu($tvar = 1)
  {
    ...
    $this->vypisovat_chybu = $this->NactiUnikatniObsah($this->unikatni["set_vypisovat_chybu_{$tvar}"]);
    ...
    for ($i = 0; $i < count($this->menu); $i++)
    {
      ...
      $ente = ($this->ente_definovane ? in_array($i, $this->ente_ozn_def) :
                                        (($i + $this->ente_ozn_od) % $this->ente_ozn_po) == 0);  //kazde 2 od 0 nebo def.
      ...
      $mainhref = (!Empty($this->menu[$i]["main_href"]) ? ($this->var->htaccess ? "{$this->absolutni_url}{$this->menu[$i]["main_href"]}" : "?{$this->var->get_kam}={$this->menu[$i]["main_href"]}") : "./");
