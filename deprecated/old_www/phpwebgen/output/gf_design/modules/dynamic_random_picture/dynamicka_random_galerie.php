 ...
 */
  private function LoadConfig(&$limit)
  {
    if (file_exists($this->conffile))
    {
      $u = fopen($this->conffile, "r");
    ...
    if (!Empty($co))
    {
      switch ($co)
      {
        ...
        case "add": //pridavani obrazku
          ...
          {
            $ob = $this->ZpracujObrazek($_FILES["obrazek"], $url, $w_mini, $h_mini, $w_full, $h_full);
          }
          ...
          if ($res = @$this->sqlite->query("SELECT adresa, popisek, url, datum, w_mini, h_mini, w_full, h_full FROM random_gallery WHERE id={$id};", NULL, $error))
          {
            if ($res->numRows() == 1)
            {
              ...
              $result = $this->NactiUnikatniObsah($this->unikatni["admin_edit"],
                                                  ...
                                                  ($data->w_full == 0 && $data->h_full == 0 ? " checked=\"checked\"" : ""),
                                                  ($data->w_mini != 0 && $data->h_mini != 0 ? "mini_3();" : ($data->h_mini != 0 ? "mini_2();" : ($data->w_mini != 0 ? "mini_1();" : ""))),
              ...
              if (!Empty($_FILES["obrazek"]["tmp_name"]))
              {
                $this->ZpracujObrazek($_FILES["obrazek"], $url, $w_mini, $h_mini, $w_full, $h_full);
              }
                else
              {
                $url = $data->url;
 ...
 */
  private function ZpracujObrazek($tmp, &$obrazek, $w_mini, $h_mini, $w_full, $h_full)
  {
    ...
    if (($this->ValueConfig(0) != 0 ? $tmp["size"] < (1024 * 1024 * $this->ValueConfig(0)) : true) && //je-li nastaven limit omezuje jinak pousti
        $result)
    {
