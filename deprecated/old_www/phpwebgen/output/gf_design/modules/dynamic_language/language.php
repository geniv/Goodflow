    ...
    if ($res = @$this->sqlite->query("SELECT id, jazyk, zkratka FROM jazyk ORDER BY LOWER(zkratka) ASC;", NULL, $error))
    {
      if ($res->numRows() != 0)
      {
        while ($data = $res->fetchObject())
        {
          ...
          $result .= $this->NactiUnikatniObsah($this->unikatni["normal_seznam_jazyku_{$tvar}"],
                                              ($this->var->htaccess ? $this->NactiUnikatniObsah($this->unikatni["rewrite_url_seznam_jazyku_{$tvar}"],
                                                                                                $this->absolutni_url,
                                                                                                $data->zkratka) : "?{$this->var->get_submenu}=changelang&amp;id={$data->id}"),
                                              $data->jazyk,
                                              $data->zkratka,
                                              ($podm ? $this->ozn_jazyk_l : ""),
                                              ($podm ? $this->ozn_jazyk_r : ""),
                                              $ozn_class);
        }
      }
        else
