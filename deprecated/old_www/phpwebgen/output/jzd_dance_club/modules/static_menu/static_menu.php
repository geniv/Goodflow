 ...
 */
  public function Menu($tvar = 1)
  {
    ...
    for ($i = 0; $i < count($this->menu); $i++)
    {
      ...
      $mainhref = (!Empty($this->menu[$i]["main_href"]) ? ($this->var->htaccess ? "{$this->absolutni_url}{$this->menu[$i]["main_href"]}" : "?{$this->var->get_kam}={$this->menu[$i]["main_href"]}") : $this->absolutni_url);
