 ...
 */
  public function Prepinani($adr = NULL, $kontrola = true)
  {
    if (!Empty($adr))
    {
      $adresa = $adr;
    ...
    if ($res = @$this->sqlite->query("SELECT kod FROM prepinani WHERE adresa='{$adresa}';", NULL, $error))
    {
      if ($res->numRows() == 1)
      {
        $result = ($kontrola ? htmlspecialchars_decode(html_entity_decode($res->fetchObject()->kod, ENT_QUOTES)) : $res->fetchObject()->kod);
      }
    }
      else
    {
