        ...
        case "addobsah":  //pridavani obsahu
              ...
              $poc == $pocetelem)
          {
            ...
            for ($i = 0; $i < $pocetelem; $i++)
            {
              switch ($podminka[$i]["typ"])
              {
                ...
                default:  //rozdeleni hodnot
                  $ukladani[$i] = $_POST[$podminka[$i]["name"]];
                break;
              }
            }
            ...
            if (@$this->sqlite->queryExec("INSERT INTO obsah_sablony (id, sablona, obsah, pridano, nazev, rewrite) VALUES
                                          (NULL, {$sablona}, '{$ulozit}', datetime('now', '+2 hour'), '{$nazev}', '{$rewrite}');", $error))
            {
              $navic = $this->SyncFileWithDB();
              $result = $this->NactiUnikatniObsah($this->unikatni["admin_addobsah_hlaska"], $ulozit, $navic);
            }
              else
            {
              $this->var->main[0]->ErrorMsg($error);
...
*/
              ...
              $id != 0)
          {
            ...
            {
              $navic = $this->SyncFileWithDB();
              $result = $this->NactiUnikatniObsah($this->unikatni["admin_editobsah_hlaska"], $ulozit, $navic);
            }
              else
            {
              $this->var->main[0]->ErrorMsg($error);
        ...
        case "delobsah": //mazani podle id obsahu
          ...
          if ($res = @$this->sqlite->query("SELECT obsah FROM obsah_sablony WHERE id={$id};", NULL, $error))
          {
            if ($res->numRows() == 1)
            {
              ...
              {
                $navic = $this->SyncFileWithDB();
                $result = $this->NactiUnikatniObsah($this->unikatni["admin_delobsah_hlaska"], $data->obsah, $navic);
              }
                else
              {
                $this->var->main[0]->ErrorMsg($error);
                                      ...
                                      ORDER BY obsah_sablony.pridano DESC;", NULL, $error))
    {
      if ($res->numRows() != 0)
      {
        while($data = $res->fetchObject())
        {
          $result .=
          "
          {$data->obsah}, {$data->pridano}, {$data->nazev}, {$data->rewrite}<br />
