 ...
 *
 * pouziti: <strong>$text = $this->var->main[0]->NactiFunkci("DynamicForm", "Formular"[, 1, $captcha, $slovo, "adresa"]);</strong>
 *
 ...
 */
  public function Formular($adr = NULL, $idcaptcha = NULL, $captcha = NULL, $slovo = NULL, $tvar = 1)
  {
    if (!Empty($adr))
    {
      $adresa = $adr;
    ...
    if (!Empty($co))
    {
      switch ($co)
      {
        ...
        case "addform": //pridavani formulare
          $result = $this->NactiUnikatniObsah($this->unikatni["admin_add_form"]); //vlozit moznosti captcha kodu
          ...
          //propracovat na kontrolu vic jak 1 emailu zaroven!!!
        ...
        case "addelem": //pridavani elementu
          ...
          //var_dump($num_input[$type]);
          $jmeno_typu = array_keys($this->input);
          $jm_typ = $jmeno_typu[$type];
          ...
          $result = $this->NactiUnikatniObsah($this->unikatni["admin_add_elem"],
                                              ...
                                              (!Empty($num_input[$type][10]) ? $this->NactiUnikatniObsah($this->unikatni["admin_add_elem_vstupni_typ"],
                                                                                                        $this->VyberVstupu($vstup, "?{$this->var->get_kam}={$this->var->adresaadminu}&amp;{$this->var->get_idmodul}={$this->idmodul}&amp;co=addelem&amp;form={$form}&amp;typ={$type}")) : ""),
          ...
          if (!Empty($_POST["tlacitko"]) &&
              ...
              $poradi > 0)
          {
            if (@$this->sqlite->queryExec("INSERT INTO prvek (id, formular, nazev, typ, value, input_class, input_id, input_akce, readonly, disabled, label, zacatek, konec, povinne, reg_exp, vstupni_typ, min_poc, max_poc, poradi) VALUES
                                          (NULL, {$formular}, '{$nazev}', {$typ}, '{$value}{$src}', '{$input_class}', '{$input_id}', '{$input_akce}', {$readonly}, {$disabled}, {$label}, {$zacatek}, {$konec}, {$povinne}, '{$reg_exp}', {$vstupni_typ}, {$min_poc}, {$max_poc}, {$poradi});", $error))
            {
              $result = $this->NactiUnikatniObsah($this->unikatni["admin_add_elem2_hlaska"], $nazev);
            }
              else
            {
              $this->var->main[0]->ErrorMsg($error);
        ...
        case "editelem":  //editace elementu
          ...
          if ($res = @$this->sqlite->query("SELECT formular, nazev, typ, value, input_class, input_id, input_akce, readonly, disabled, label, zacatek, konec, povinne, reg_exp, vstupni_typ, min_poc, max_poc, poradi FROM prvek WHERE id={$id};", NULL, $error))
          {
            if ($res->numRows() == 1)
            {
              $data = $res->fetchObject();
