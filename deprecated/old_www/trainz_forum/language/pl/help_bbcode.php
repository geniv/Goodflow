...
*
* @package language
* @version $Id: help_bbcode.php,v 1.27.1 2007/12/28 Ronnie Exp $
...
$help = array(
	...
	array(
		0 => 'Czym jest BBCode?',
		1 => 'BBCode jest specjalną implementacją HTML\'a, a możliwość jego używania jest uzależniona od ustawień dokonanych przez administratora. Jeśli chcesz, możesz wyłączyć go w niektórych postach, zaznaczając opcję Wyłącz BBCode przy wysyłaniu ich. Znaczniki BBCode\'u są zamknięte w nawiasach kwadratowych - [ i ], a nie w trójkątnych - &lt; i &gt;, oraz oferują większą kontrolę nad tym co i jak będzie wyświetlane. Zależnie od szablonu, którego używasz, możesz w bardzo łatwy sposób dodawać znaczniki BBCode do postów poprzez odpowiednie przyciski na stronie wysyłania postu. Mimo to ten przewodnik może Ci się przydać.
		<style type="text/css">
		<!--
	...
	array(
		...
		1 => 'Są dwa sposoby na cytowanie tekstu - podając źródło lub nie podając go.<ul>
			<li>Przy wykorzystywaniu funkcji cytowania odpowiadając na post na forum, cytowany tekst zostaje automatycznie otoczony blokiem <code><strong>[quote=&quot;</strong><span style="font-style: italic;">autor cytowanego posta</span><strong>&quot;]</strong></code> i <code><strong>[/quote]</strong></code>. Ta metoda pozwala cytować z podaniem źródła (czyli np. osoby). Przykładowo, żeby zacytować kawałek tekstu napisanego przez Userka powinieneś napisać: <code><strong>[quote=&quot;</strong>Userek<strong>&quot;]</strong>Tekst, który napisał Userek<strong>[/quote]</strong></code> Wynikiem tego będzie automatyczne dodanie „Userek napisał(a):” przed cytowanym tekstem. Pamiętaj, <strong>musisz</strong> wstawić cudzysłowy wokół nazwy źródła.</li>
			<li>Druga metoda pozwala cytować tekst nie podając źródła. Aby jej użyć, wstaw tekst między znacznikami <code><strong>[quote]</strong></code> i <code><strong>[/quote]</strong></code>. Przy wyświetlaniu posta, przed cytowanym tekstem wyświetlony zostanie napis &quot;Cytat:&quot;.</li>
		</ul>'
	),
	array(
		0 => 'Wstawianie kodu lub tekstu o stałej szerokości znaków',
		1 => 'Jeśli chcesz wstawić kawałek kodu lub cokolwiek, co wymaga stałej szerokości znaków, powinieneś użyć znaczników <code><strong>[code]</strong></code> i <code><strong>[/code]</strong></code>. Przykład: <code><strong>[code]</strong>echo &quot;Oto jakiś kod&quot;;<strong>[/code]</strong></code>. Całe formatowanie użyte między znacznikami <code><strong>[code]</strong></code> i <code><strong>[/code]</strong></code> zostanie zachowane przy wyświetlaniu wysłanego posta. Kolorowanie składni PHP może zostać włączone przez użycie pary <code><strong>[code=php]</strong></code> i <code><strong>[/code]</strong></code> i jest zalecane przy wysyłaniu kodu PHP, ponieważ ułatwia jego odczytywanie.'
	),
	array(
		0 => '--',
		1 => 'Tworzenie list'
	...
	array(
		...
		<ol style="list-style-type: arabic-numbers" class="faq-block">
			<li>Idź do sklepu</li>
		...
		<strong>[/list]</strong></code>
		, żeby zrobić listę możliwych odpowiedzi:
	...
	array(
		...
		1 => 'Załączniki moga być wstawiane do postów za pomocą nowej pary znaczników BBcode - <code><strong>[attachment=]</strong></code> i <code><strong>[/attachment]</strong></code>. Aby ów znacznik działał, wysyłanie i ściąganie załączników musi zostać włączone w Panelu Administracyjnym, a Ty musisz posiadać odpowiednie uprawnienia. W formularzu wysyłania posta jest lista wyboru służąca do wstawiania załączników w treści posta.'
	),
	array(
		0 => '--',
		1 => 'Inne sprawy'
