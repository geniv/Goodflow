	...
	foreach ($group_data as $group_id => $data)
	{
		...
		foreach ($check as $column => $value_ary)
		{
			...
			foreach ($value_ary as $user_id => $value)
			{
				...
				*/ 
				if (($value > (int) $data['group_max_' . $column] && (int) $data['group_max_' . $column] != 0) || $value < (int) $data['group_min_' . $column])
				{
				...
				*/
				else if ($value >= (int) $data['group_min_' . $column] && ($value < (int) $data['group_max_' . $column] || (int) $data['group_max_' . $column] == 0) && (!isset($remove_groups[$group_id]) || (isset($remove_groups[$group_id]) && !in_array($user_id, $remove_groups[$group_id]))))
				{
