...
class acp_board
{
	...
	function main($id, $mode)
	{
      ...
      'legend3'				=> 'ACP_ANNOUCEMENTS',
            ...
            // USER ACCOUNT VERIFICATION BY FOREIGN DATABASE // (c) JediSoft.cz, 2008
						'duv_legend'		 => 'DUV_LEGEND',
						'duv_server'		 => array('lang' => 'DUV_SERVER', 'validate' => 'text',	'type' => 'text:20:60', 'explain' => true),
						'duv_dbuser'		 => array('lang' => 'DUV_DBUSER', 'validate' => 'text',	'type' => 'text:20:60', 'explain' => true),
						'duv_dbpasswd'	 => array('lang' => 'DUV_DBPASSWD', 'validate' => 'text',	'type' => 'password:20:60', 'explain' => true),
						'duv_database'	 => array('lang' => 'DUV_DATABASE', 'validate' => 'text',	'type' => 'text:20:60', 'explain' => true),
						'duv_dbtable'	   => array('lang' => 'DUV_DBTABLE', 'validate' => 'text',	'type' => 'text:20:60', 'explain' => true),
						'duv_ta_user'	   => array('lang' => 'DUV_TA_USER', 'validate' => 'text',	'type' => 'text:20:60', 'explain' => true),
						'duv_ta_passwd'  => array('lang' => 'DUV_TA_PASSWD', 'validate' => 'text',	'type' => 'text:20:60', 'explain' => true),
						'duv_tau_verify' => array('lang' => 'DUV_TAU_VERIFY', 'validate' => 'text',	'type' => 'select', 'method' => 'select_tau_verify', 'explain' => true),
						'duv_tap_verify' => array('lang' => 'DUV_TAP_VERIFY', 'validate' => 'text',	'type' => 'select', 'method' => 'select_tap_verify', 'explain' => true),
						'duv_copy'       => array('lang' => 'DUV_COPY', 'validate' => 'text',	'type' => 'textarea:5:40', 'explain' => true),
						'duv_sql'        => array('lang' => 'DUV_SQL', 'validate' => 'text',	'type' => 'text:70:255', 'explain' => true),
						'duv_encoding'   => array('lang' => 'DUV_ENCODING', 'validate' => 'text',	'type' => 'select', 'method' => 'select_encoding', 'explain' => true),
	...
	*/
	function select_acc_activation($value, $key = '')
	{
		...
		$radio_ary += array(USER_ACTIVATION_DATABASE => 'ACC_DATABASE');
	...
	*/
	...
	/**
	* DUV Select of comparation method of username
	*/
	function select_tau_verify($selected_value, $key)
	{
	  global $user;
	  
		$ary = array(0=> 'DUV_SEL_LIKE', 1=> 'DUV_SEL_EQUALS');
		$options = '';
		foreach ($ary as $key => $type)
		{
			$selected = ($selected_value == $key) ? ' selected="selected"' : '';
			$options .= '<option value="' . $key . '"' . $selected . '>' . $user->lang[$type] . '</option>';
		}
	...
	/**
	* DUV Select of comparation method of password
	*/
	function select_tap_verify($selected_value, $key)
	{
	  global $user;
	  
		$ary = array(0=> 'DUV_SEL_PLAIN', 1=> 'DUV_SEL_MD5', 2=> 'DUV_SEL_OLDPASSWORD', 3=> 'DUV_SEL_PASSWORD');
		$options = '';
		foreach ($ary as $key => $type)
		{
			$selected = ($selected_value == $key) ? ' selected="selected"' : '';
	...
	/**
	* DUV Select text encoding
	*/
	function select_encoding($selected_value, $key)
	{
	  global $user;

		$ary = array('ISO-8859-2' => 'ISO 8859-2', 'windows-1250' => 'Windows 1250', 'UTF-8' => 'UTF-8');
		$options = '';
		foreach ($ary as $key => $type)
		{
			$selected = ($selected_value == $key) ? ' selected="selected"' : '';
