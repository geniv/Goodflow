...
class acm
{
	...
	*/
	function tidy()
	{
		...
		{
			if (!preg_match('/^(sql_|feed_|data_(?!global))/', $entry))
			{
				continue;
			}
	...
	*/
	function purge()
	{
		...
		while (($entry = readdir($dir)) !== false)
		{
			if (strpos($entry, 'sql_') !== 0 && strpos($entry, 'data_') !== 0 && strpos($entry, 'ctpl_') !== 0 && strpos($entry, 'tpl_') !== 0  && strpos($entry, 'feed_') !== 0)
			{
				continue;
			}
	...
	*/
	function feed_load($identifier)
	{
		global $phpEx;

		if (!file_exists($this->cache_dir . 'feed_' . $identifier . ".$phpEx"))
		{
			return false;
		}

		@include($this->cache_dir . 'feed_' . $identifier . ".$phpEx");

		if ($expired)
		{
			@unlink($this->cache_dir . 'feed_' . $identifier . ".$phpEx");
			return false;
		}
	...
	* Save feed data
	*/
	function feed_save($feed_data, $identifier, $ttl)
	{
		global $phpEx;

		if ($fp = @fopen($this->cache_dir . 'feed_' . $identifier . '.' . $phpEx, 'wb'))
		{
			@flock($fp, LOCK_EX);

			$file = "<?php\n";
			$file .= "\$expired = (time() > " . (time() + $ttl) . ") ? true : false;\nif (\$expired) { return; }\n";

			fwrite($fp, $file . "\n\$feed_data = " . var_export($feed_data, true) . ";\n?>");
			@flock($fp, LOCK_UN);
			fclose($fp);
		}
	}
