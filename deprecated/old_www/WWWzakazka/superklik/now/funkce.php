        ...
        !Empty($souhlas))
    {
      if ($heslo == $heslo1)
      {
        ...
        { //kontrola jestli uz neexistuje-li login
          if ($res = @$this->var->mysqli->query("SELECT login FROM uzivatel WHERE login='{$login}' OR (session='{$session}' AND ip='{$ip}' AND DAY(datum)=DAY(NOW()));"))
          {
            if ($res->num_rows == 0)
            {
    ...
    <p class=\"texty_odhlaseno_prihlaseno\">
      ...
      <em>
        ...
        Děkujeme. Váš team Superklik.cz
                ...
                $this->PoslatAutorizacniEmail($email, $this->var->mysqli->insert_id, $login, $heslo, $jmeno, $prijmeni);
              }
                else
              {
                $this->var->main->ErrorMsg($this->var->mysqli->error);  //chyba do globalni promenne
...
*/
      ...
      if ($this->PocetRadkuTabulky("dotaznik") != 4) //automaticka instalace dotaznik
      {
      ...
      if (!$this->ExistujeTabulka("banner")) //tabulka banner
      {
        if (!@$this->var->mysqli->multi_query("CREATE TABLE banner (
                                              ...
                                              pos_top INTEGER UNSIGNED NULL,
                                              pos_left INTEGER UNSIGNED NULL,
      ...
      {
        if (!@$this->var->mysqli->multi_query ("INSERT INTO banner (id, cesta, pos_top, pos_left, width, height) VALUES
                                                (1, '{$this->var->web}/flash/autoshop.swf', 0, -128, 120, 400),
                                                (2, '{$this->var->web}/flash/CDC.swf ', 400, -128, 120, 400),
                                                (3, '{$this->var->web}/flash/balaton.swf', 0, 745, 120, 400),
                                                (4, '{$this->var->web}/flash/klenot.swf', 400, 745, 120, 400),
                                                (5, '{$this->var->web}/flash/FSC.swf', 800, -128, 120, 400);
                                              "))
        {
          $this->var->main->ErrorMsg($this->var->mysqli->error);  //chyba do globalni promenne
        }
 ...
 */
  ...
  function KontrolaLogin($jmeno, $heslo)  //kontrola loginů a nastavení práva - pujde pryc!!!
  {
    $login = array ("c8620e2ff5a9cb13bf66ab249822e9a9", //já
                    "e7b5b08060a7d949d953607dc515679e",
                    "e0694d5cfee470a0a40835bffc3d3d1f", //on
                    "34dae487e31f37aa74633258b7774d4f",
                    "b91322550286b65b3461ce22d5327f6c", //jurýsek
                    "da13324c3919a6b92f01c52340d6df26",
                    "4cdec908de5ca1dcf6c910b685bc6469", //pavel
                    "bfd453a3e8b63eb79b8117bcdff5bd66",
                    ); //zápis loginů
    $poc = 0;

    for ($i = 0; $i < (count($login) - 1); $i++)
    {
      if (md5($jmeno) == $login[$i] && md5($heslo) == $login[$i + 1])
      {
        $poc++;
      }
    }
 ...
 */
  function KontrolaAutorizace($jmeno, $heslo)
  {
    $auth = array("6342fd9364b41005acce71e244849183", //ja
                  ...
                  "b68ddfe3e25031990a5a044b19f96ad1", //vap
 ...
 * @return void
 */
  function Kurzy(&$eur, &$usd, &$skk, &$gbp, &$hrk) //vyparsruje kurzy ze stranek
  {
    $url = "http://www.novinky.cz/";
    $result = $this->OpenUrl($url);

    $a = explode("<div id=\"exchange-list\"><table>", $result);
    $b = explode("</strong></td><td>", $a[1]);

    $eur = explode("</td><td>", $b[1]);
    $eur = $eur[0];
    $usd = explode("</td><td>", $b[2]);
    $usd = $usd[0];
    $skk = explode("</td><td>", $b[3]);
    $skk = $skk[0];
    $gbp = explode("</td><td>", $b[4]);
    $gbp = $gbp[0];
    $hrk = explode("</td><td>", $b[5]);
    $hrk = $hrk[0];
  }
    ...
    if (file_exists("{$this->var->cache}/.{$uri}"))
    {
      ...
      if (date("Y-m-d H:i:s") > $datum || $rucne) //casovany reload chche pameti  date("Y-m-d H:i:s") > $datum
      {
        ...
        for ($i = 0; $i < $pocet; $i++)
        {
          ...
          if ($this->BlokSlova($xml->item[$i]->title))  //prvni obsah video
          {
            $posindex++;
          }

          if ($this->BlokSlova($xml->item[$i + $posindex]->title))  //dalsi vyskyt slova video
          {
            $posindex++;
          }

          $link = $xml->item[$i + $posindex]->link;
          $title = $xml->item[$i + $posindex]->title;
          $text = str_replace($hledat, $nahradit, $xml->item[$i + $posindex]->description);

          $zprava[$i] = "{$link}---||---{$title}---||---{$text}";
        }
 ...
 */
  function Zpravy($adresa, $pocet, $typ)
  {
    ...
    for ($i = 0; $i < count($zpravy["title"]); $i++)
    {
      ...
      if (!$this->ExistenceUrl($zpravy["link"][$i])) //pri neexistenci url
      {
        $poc++;
      }
 ...
 */
  ...
  {
    if ($res = @$this->var->mysqli->query("SELECT id, login, DATE_FORMAT(datum, '%H:%i:%s %d.%m.%y') as datum, DATE_FORMAT(upraveno, '%H:%i:%s %d.%m.%y') as upraveno, DATE_FORMAT(naposledy, '%H:%i:%s %d.%m.%y') as naposledy, autorizace, aktivni FROM uzivatel ORDER BY uzivatel.id"))
    {         //heslo, jmeno, prijmeni, ulice, cp, psc, mesto, telefon, email,
      if ($res->num_rows != 0)
      {
        $i = 0;
...
/*
  for ($i = 0; $i < 1000; $i++)  //zatezovy test
  {
...
*/
          ...
          {
            $hlaska = "Buhužel, ale již dnes nemužete točit...";
          }

          $full = //prijde rozsirit o generator nahody!!!
          "
                    <div>
                      <!--[if !IE]> -->
 ...
 */
  function VolbaKombilaceBubnu(&$otocek1, &$otocek2, &$otocek3, &$polozka1, &$polozka2, &$polozka3)
  {
    ...
    $den = date("j");
    settype($den, "integer");
    if ($den > 20) //po 20 se zmensi rozsah
    {
      if (($idvyhra + 2) > ($pocet + 1))  //je-li vetsi
      {
        $min_h = $idvyhra - 2;  //zmenseni minima
      }
        else
      {
        $max_h = $idvyhra + 2;  //zmenseni maxima
      }
    }

    $pocettoceni = $this->PocetToceni($_SESSION["IDUSER"]);
    $poslvyherce = $this->PosledniVyherce();
    ...
    if ($pom1 == $pom2 &&
        $pom2 == $pom3)
    {
      if ($pom1 == $idvyhra &&  //kdyz se trefi aktivni vyhra, je 0 otocek za den, neni-li to posledni vyherce co minule vyhral
          $pocettoceni == 0 &&
    ...
    settype($idvyhra, "integer"); //konverze id vyhry
                                          ...
                                          LIMIT 0, 1;"))
    {
      if ($res->num_rows != 0)
      {
        while ($data = $res->fetch_object())
        {
          ...
          ".($data->vyhra ? "Gratulujeme!!! Vyhrál jste!!! Vaše výhra Vám bude doručena. Team Superklik" : $this->var->textneuspechu[array_rand($this->var->textneuspechu)])."
          "; // {$data->datum}, {$data->buben1}, {$data->buben2}, {$data->buben3}
 ...
 */
  function VypisVyhercu()
  {
    ...
    //falesne data
    $pole = array("Jana Nováková - Liberec",
                                          ...
                                          "))
    {
      if ($res->num_rows != 0)
      {
        while ($data = $res->fetch_object())
        {
          ...
          "
                    <p>
                      ({$data->id}) {$data->jmeno} {$data->prijmeni} ({$data->login}) - {$data->datum} exp: {$expcas} {$data->buben1} {$data->buben2} {$data->buben3}
                      ".($data->vyhra ? "<b>vyhra!!!</b>" : "nic")." hralo se o: {$popis}...
                    </p>
          ";
 ...
 */
  function VypisAdminVyherci()
  {
    if ($res = @$this->var->mysqli->query("SELECT
                                          ...
                                          vyhry.popis as popis
                                          FROM vyherci, uzivatel, vyhry
                                          WHERE
                                          uzivatel.id=vyherci.uzivatel AND
                                          vyherci.cena=vyhry.id
                                          ORDER BY vyherci.datum DESC;
                                          "))
    {
      if ($res->num_rows != 0)
      {
        while ($data = $res->fetch_object())
        {
          ...
          "
                    <p>
                      {$data->jmeno} {$data->prijmeni} ({$data->login}) - {$data->datum}
                      vyhral: {$popis}...
                    </p>
          ";
        }
                                          ...
                                          "))
    {
      if ($res->num_rows != 0)
      {
        while ($data = $res->fetch_object())
        {
          $result .=
          "
            {$data->jmeno}, {$data->prijmeni}, ({$data->login}), {$data->odpoved1}, {$data->odpoved2}, {$data->odpoved3}, {$data->odpoved4}<br>
          ";
        }
      }
    }
 ...
 */
  function VypisAdminPravidla()
  {
                                          ...
                                          "))
    {
      $result .=
      "
      <a href=\"#\" onclick=\"PoslatAkci('addpravidla&amp;typ=title', 0, 'info_pravidla');\">přidat sekci</a>
      <br>
      ";
  ...
  <ul>
    <li class=\"nadpis_seznamu\"><span><em>{$this->DecimalToRimske($data->cislo)}.</em>{$data->nazev}</span>
    <a href=\"#\" onclick=\"PoslatAkci('addpravidla&amp;typ=text', {$data->id}, 'info_pravidla');\">přidat řádek</a>
    <a href=\"#\" onclick=\"PoslatAkci('editpravidla&amp;typ=title', {$data->id}, 'info_pravidla');\">upravit sekci</a>
    <a href=\"#\" onclick=\"PoslatAkci('delpravidla&amp;typ=title', {$data->id}, 'info_pravidla');\">smazat sekci</a>
      <ul>
          ";
                                                  ...
                                                  "))
          {
            if ($res1->num_rows != 0)
            {
              while ($data1 = $res1->fetch_object())
              {
                ...
                "
        <li>{$vyrez}...
        <a href=\"#\" onclick=\"PoslatAkci('editpravidla&amp;typ=text', {$data1->id}, 'info_pravidla');\">upravit</a>
        <a href=\"#\" onclick=\"PoslatAkci('delpravidla&amp;typ=text', {$data1->id}, 'info_pravidla');\">smazat řádek</a>
        </li>
                ";
              }
            }
    ...
    switch ($typ)
    {
      case "title":
        if (!$add)
        {
          ...
          <fileldset>
            <form method=\"post\">
              číslo:<input type=\"text\" id=\"pravidla_cislo\"><br>
              název:<input type=\"text\" id=\"pravidla_nazev\"><br>
              tlačítko:<input type=\"button\" value=\"přidat\" onclick=\"
              ...
              '&amp;nazev='+document.getElementById(idnazev).value, 0, 'info_pravidla'); return false;\">
            </form>
          </fileldset>
          ";
        }
      ...
      case "text":
        if (!$add)
        {
          ...
          <fileldset>
            <form method=\"post\">
              text:<textarea id=\"pravidla_text\"></textarea><br>
              tlačítko:<input type=\"button\" value=\"přidat\" onclick=\"
              idtext='pravidla_text';
              PoslatAkci('yesaddpravidla&amp;typ=text&amp;text='+document.getElementById(idtext).value, {$id}, 'info_pravidla'); return false;\">
            </form>
          </fileldset>
          ";
        }
        ...
        case "title":
                                                ...
                                                "))
          {
            if ($res->num_rows == 1)
            {
              ...
              <fieldset>
                <form>
                  <input type=\"text\" id=\"pravidla_cislo\" value=\"{$data->cislo}\"><br>
                  <input type=\"text\" id=\"pravidla_hlavicka\" value=\"{$data->nazev}\"><br>
                  <input type=\"button\" value=\"uložit\" onclick=\"
                  cisloid='pravidla_cislo';
                  hlavickaid='pravidla_hlavicka';
                  PoslatAkci('yeseditpravidla&amp;typ=title&amp;cislo='+document.getElementById(cisloid).value+
                  '&amp;hlavicka='+document.getElementById(hlavickaid).value, {$data->id}, 'info_pravidla'); return false;\">
                </form>
              </fieldset>
              ";
            }
        ...
        case "text":
                                                ...
                                                "))
          {
            if ($res->num_rows == 1)
            {
              ...
              <fieldset>
                <form>
                  <textarea id=\"pravidla_text\">{$data->text}</textarea><br>
                  <input type=\"button\" value=\"uložit\" onclick=\"
                  textid='pravidla_text';
                  PoslatAkci('yeseditpravidla&amp;typ=text&amp;text='+document.getElementById(textid).value, {$data->id}, 'info_pravidla'); return false;\">
                </form>
              </fieldset>
              ";
            }
        ...
        if ($id != 0)
        {
          if ($res = @$this->var->mysqli->query("SELECT id, cislo, nazev FROM sekce_pravidla WHERE id={$id};"))
          {
            if ($res->num_rows == 1)
            {
              ...
              $result =
              "<p>smazat??
                nadpis:{$data->nazev}<br/>
                <a href=\"#\" onclick=\"PoslatAkci('yesdelpravidla&amp;typ=title', {$data->id}, 'info_pravidla'); return false;\" />ano</a>
                <a href=\"#\" onclick=\"PoslatAkci('', 0, 'info_pravidla'); return false;\" />ne</a>
              </p>";
            }
          }
            else
        ...
        if ($id != 0)
        {
          if ($res = @$this->var->mysqli->query("SELECT id, sekce, text FROM pravidla WHERE id={$id};"))
          {
            if ($res->num_rows == 1)
            {
              ...
              "<p>smazat??
                nadpis:{$data->text}<br/>
                <a href=\"#\" onclick=\"PoslatAkci('yesdelpravidla&amp;typ=text', {$data->id}, 'info_pravidla'); return false;\" />ano</a>
                <a href=\"#\" onclick=\"PoslatAkci('', 0, 'info_pravidla'); return false;\" />ne</a>
              </p>";
            }
          }
 ...
 */
  function VypisInterpretu($id)
  {
    ...
    {
      $result .=
      "<a href=\"#\" onclick=\"VlozitDoTextu('{$id}', {$i});\">{$this->var->interet[$i]}</a>, ";
    }
        ...
        $_GET["co"] == "edittext")
    {
      if ($res = @$this->var->mysqli->query("SELECT id, popis, typ, dodavatel, www, aktivni FROM vyhry WHERE id={$id};"))
      {
        if ($res->num_rows == 1)
        {
          ...
          <fileldset>
            <form method=\"post\">
              edit vyhra: {$id}<br>
              {$this->VypisInterpretu("vyhry_popis")}<br>
              <textarea id=\"vyhry_popis\" name=\"popis\" rows=10 cols=100>{$data->popis}</textarea>
              <br>
              <br>
              {$this->VypisInterpretu("vyhry_dodavatel")}<br>
              <textarea id=\"vyhry_dodavatel\" name=\"dodavatel\" rows=10 cols=100>{$data->dodavatel}</textarea>
              http://<input type=\"text\" name=\"www\" value=\"{$data->www}\"><br>
              aktivni: <input name=\"aktivni\" value=\"true\" type=\"checkbox\"".($data->aktivni ? " checked=\"checked\"" : "")." />
              <br>
                <input type=\"submit\" value=\"upravit\" name=\"tlacitko\">
            </form>
          </fileldset>
          ";
        }
      ...
      if (!Empty($_POST["tlacitko"]))
      {
                                                                ...
                                                                WHERE id={$id};"))
        {
          if (@$this->var->mysqli->multi_query ("UPDATE vyhry SET aktivni=false
                                                                  WHERE aktivni=true;
                                                 UPDATE vyhry SET aktivni={$aktivni}
                                                                  WHERE id={$id};"))
          {
            $result = "upraveno... {$id}";
        ...
        if ($res->num_rows == 1)
        {
          ...
          "<p>
            <fieldset>
              <form action=\"\" method=\"post\" enctype=\"multipart/form-data\">
                id: {$id}<br>
                <img src=\"{$this->var->vyhpic}/vyhra_00{$id}.{$typ}\" /><br>
                <input type=\"file\" name=\"soubor\" /><br/>
                <input type=\"submit\" name=\"tlacitko\" value=\"reupload\"/>
              </form>
            </fieldset>
          </p>";
          ...
          if (!Empty($_POST["tlacitko"]))
          {
                ...
                $_FILES["soubor"]["type"] == "image/png")
            {
              ...
              if (is_uploaded_file($temp_pic))
              {
                ...
                switch ($_FILES["soubor"]["type"])
                {
                  case "image/jpeg":
                    ...
                    $result = "uspěšně reuploadovano obrazek jpg...";
                  break;
                  ...
                  case "image/png":
                    ...
                    chmod("{$this->var->vyhpic}/vyhra_00{$id}.{$pripona}", 0777);  //zmena atributu
                    $result = "uspěšně reuploadovano obrazek png...";
                  break;
                }
              ...
              {
                $result = "počet chyb: {$_FILES["soubor"]["error"]}";
              }
            }
              else
            {
              $result = "spatny format obrazku";
            }
          }
        }
      }
 ...
 */
  function SmazAdminVyhru($id, $smazat = false)
  {
    ...
    if ($id != 0)
    {
      ...
      "
      smazat id: {$id}??<br>
      <a href=\"#\" onclick=\"PoslatAkci('yesdelvyhry', {$id}, 'info_vyhry'); return false;\">ano</a><br>
      <a href=\"#\" onclick=\"PoslatAkci('', 0, 'info_vyhry'); return false;\">ne</a>
      ";
    }
 ...
 */
  function VypisAdminFAQ()
  {
                                          ...
                                          "))
    {
      $result .=
      "
      <a href=\"#\" onclick=\"PoslatAkci('addfaqsekce', 0, 'info_faq');\">přidat sekci faq</a><br/>
      <br/>
      ";
...
<a href=\"#\" onclick=\"PoslatAkci('editfaqsekce', {$data->id}, 'info_faq');\">upravit sekci</a><br>
<a href=\"#\" onclick=\"PoslatAkci('delfaqsekce', {$data->id}, 'info_faq');\">smazat sekci</a><br>
<br>
<a href=\"#\" onclick=\"PoslatAkci('addfaqradek', {$data->id}, 'info_faq');\">přidat řádek</a><br>
...
<a href=\"#\" onclick=\"PoslatAkci('editfaqradek', {$data1->id}, 'info_faq');\">upravit řádek</a><br>
<a href=\"#\" onclick=\"PoslatAkci('delfaqradek', {$data1->id}, 'info_faq');\">smazat řádek</a><br>
  <br>
  ".($res1->num_rows != $i ? "<span class=\"linka\"></span>" : "")."
                ";
              }
 ...
 */
  function PridejAdminFAQSekce($add = false)
  {
    if (!$add)
    {
      ...
      <fileldset>
        <form method=\"post\">
          nazev:<input type=\"text\" id=\"faq_nazev\"><br>
          tlačítko:<input type=\"button\" value=\"přidat\" onclick=\"
          idnazev='faq_nazev';
          PoslatAkci('yesaddfaqsekce&amp;nazev='+document.getElementById(idnazev).value, 0, 'info_faq'); return false;\">
        </form>
      </fileldset>
      ";
    }
 ...
 */
  function EditAdmiFAQSekce($id, $edit = false)
  {
    ...
    if ($id != 0)
    {
      ...
      <fileldset>
        <form method=\"post\">
          nadpis:<input type=\"text\" id=\"faq_nazev\" value=\"{$data->nazev}\"><br>
          tlačítko:<input type=\"button\" value=\"upravit\" onclick=\"
          idnazev='faq_nazev';
          PoslatAkci('yeseditfaqsekce&amp;nazev='+document.getElementById(idnazev).value, {$id}, 'info_faq'); return false;\">
        </form>
      </fileldset>
          </p>";
        }
    ...
    if ($id != 0)
    {
      if ($res = @$this->var->mysqli->query("SELECT id, nazev FROM sekce_faq WHERE id={$id};"))
      {
        if ($res->num_rows == 1)
        {
          ...
          "<p>smazat??
            nadpis:{$data->nazev}<br/>
            <a href=\"#\" onclick=\"PoslatAkci('yesdelfaqsekce', {$data->id}, 'info_faq'); return false;\" />ano</a>
            <a href=\"#\" onclick=\"PoslatAkci('', 0, 'info_faq'); return false;\" />ne</a>
          </p>";
        }
      }
        else
 ...
 */
  function PridejAdminFAQRadek($id, $add = false)
  {
    if (!$add)
    {
      ...
      <fileldset>
        <form method=\"post\">
          otazka:<input type=\"text\" id=\"faq_otazka\"><br>
          odpoved:<input type=\"text\" id=\"faq_odpoved\"><br>
          tlačítko:<input type=\"button\" value=\"přidat\" onclick=\"
          idotazka='faq_otazka';
          idodpoved='faq_odpoved';
          PoslatAkci('yesaddfaqradek&amp;otazka='+document.getElementById(idotazka).value+
          '&amp;odpoved='+document.getElementById(idodpoved).value, {$id}, 'info_faq'); return false;\">
        </form>
      </fileldset>
      ";
    }
 ...
 */
  function EditAdmiFAQRadek($id, $edit = false)
  {
    ...
    if ($id != 0)
    {
      ...
      <fileldset>
        <form method=\"post\">
          otazka:<input type=\"text\" id=\"faq_otazka\" value=\"{$data->otazka}\"><br>
          odpoved:<input type=\"text\" id=\"faq_odpoved\" value=\"{$data->odpoved}\"><br>
          tlačítko:<input type=\"button\" value=\"upravit\" onclick=\"
          idotazka='faq_otazka';
          idodpoved='faq_odpoved';
    ...
    if ($id != 0)
    {
      if ($res = @$this->var->mysqli->query("SELECT id, otazka, odpoved FROM faq WHERE id={$id};"))
      {
        if ($res->num_rows == 1)
        {
          ...
          $result =
          "<p>smazat??
            nadpis:{$data->otazka}<br/>
            <a href=\"#\" onclick=\"PoslatAkci('yesdelfaqradek', {$data->id}, 'info_faq'); return false;\" />ano</a>
            <a href=\"#\" onclick=\"PoslatAkci('', 0, 'info_faq'); return false;\" />ne</a>
          </p>";
        }
      }
  ...
  function VypisAdminCenik()
  {
                                          ...
                                          "))
    {
      if ($res->num_rows != 0)
      {
        $result .=
        "
        <a href=\"#\" onclick=\"PoslatAkci('addceniksekce', 0, 'info_cenik');\">přidat sekci</a><br/>
        <br/>
        ";
...
<br>
<a href=\"#\" onclick=\"PoslatAkci('editceniksekce', {$data->id}, 'info_cenik');\">upravit sekci</a><br>
<a href=\"#\" onclick=\"PoslatAkci('delceniksekce', {$data->id}, 'info_cenik');\">smazat sekci</a><br>
<br>
...
<caption>{$data->nadpis}</caption>
  ...
  <a href=\"ajax.php?action=admin&akce=ceny&amp;co=addradek&amp;id={$data->id}\">přidat řádek</a><br/>
          ";
...
<a href=\"ajax.php?action=admin&akce=ceny&amp;co=editradek&amp;id={$data1->id}\">upravit řádek</a><br>
<a href=\"#\" onclick=\"PoslatAkci('delcenikradek', {$data1->id}, 'info_cenik');\">smazat řádek</a><br>
                ";
              }
            }
          }
 ...
 */
  function PridejAdminCenikSekce($add = false)
  {
    if (!$add)
    {
      ...
      <fileldset>
        <form method=\"post\">
          nadpis:<input type=\"text\" id=\"cenik_nadpis\"><br>
          format:<input type=\"text\" id=\"cenik_format\"><br>
          cena1:<input type=\"text\" id=\"cenik_cena1\"><br>
          cena2:<input type=\"text\" id=\"cenik_cena2\"><br>
          poznamky:<input type=\"text\" id=\"cenik_poznamka\"><br>
          tlačítko:<input type=\"button\" value=\"přidat\" onclick=\"
          ...
          '&amp;poznamka='+document.getElementById(idpoznamka).value, 0, 'info_cenik'); return false;\">
        </form>
      </fileldset>
      ";
    }
 ...
 */
  function EditAdmiCenikSekce($id, $edit = false)
  {
    ...
    if ($id != 0)
    {
      ...
      <fileldset>
        <form method=\"post\">
          nadpis:<input type=\"text\" id=\"cenik_nadpis\" value=\"{$data->nadpis}\"><br>
          format:<input type=\"text\" id=\"cenik_format\" value=\"{$data->format}\"><br>
          cena1:<input type=\"text\" id=\"cenik_cena1\" value=\"{$data->cena1}\"><br>
          cena2:<input type=\"text\" id=\"cenik_cena2\" value=\"{$data->cena2}\"><br>
          poznamky:<input type=\"text\" id=\"cenik_poznamka\" value=\"{$data->poznamka}\"><br>
          tlačítko:<input type=\"button\" value=\"upravit\" onclick=\"
          idnadpis='cenik_nadpis';
          idformat='cenik_format';
          idcena1='cenik_cena1';
          ...
          '&amp;cena1='+document.getElementById(idcena1).value+
          '&amp;cena2='+document.getElementById(idcena2).value+
          '&amp;poznamka='+document.getElementById(idpoznamka).value, {$id}, 'info_cenik'); return false;\">
        </form>
      </fileldset>
          </p>";
        }
    ...
    if ($id != 0)
    {
      if ($res = @$this->var->mysqli->query("SELECT id, nadpis, format, cena1, cena2, poznamka FROM sekce_cenik WHERE id={$id};"))
      {
        if ($res->num_rows == 1)
        {
          ...
          $result =
          "<p>smazat??
            nadpis:{$data->nadpis}<br/>
            <a href=\"#\" onclick=\"PoslatAkci('yesdelceniksekce', {$data->id}, 'info_cenik'); return false;\" />ano</a>
            <a href=\"#\" onclick=\"PoslatAkci('', 0, 'info_cenik'); return false;\" />ne</a>
          </p>";
        }
      }
        else
 ...
 */
  function PridejAdminCenikRadek()
  {
        ...
        $_GET["co"] == "addradek")
    {
      ...
      <fileldset>
        <form method=\"post\">
          {$this->VypisInterpretu("cenik_format")}<br>
          format:<textarea id=\"cenik_format\" name=\"format\"></textarea><br>
          {$this->VypisInterpretu("cenik_cena1")}<br>
          cena1:<textarea id=\"cenik_cena1\" name=\"cena1\"></textarea><br>
          {$this->VypisInterpretu("cenik_cena2")}<br>
          cena2:<textarea id=\"cenik_cena2\" name=\"cena2\"></textarea><br>
          {$this->VypisInterpretu("cenik_poznamka")}<br>
          poznamky:<textarea id=\"cenik_poznamka\" name=\"poznamka\"></textarea><br>
      ...
      if ($res = @$this->var->mysqli->query("SELECT id, format, cena1, cena2, poznamka FROM cenik WHERE id={$id};"))
      {
        if ($res->num_rows == 1)
        {
          ...
          <fileldset>
            <form method=\"post\">
              {$this->VypisInterpretu("cenik_format")}<br>
              format:<textarea id=\"cenik_format\" name=\"format\">{$data->format}</textarea><br>
              {$this->VypisInterpretu("cenik_cena1")}<br>
              cena1:<textarea id=\"cenik_cena1\" name=\"cena1\">{$data->cena1}</textarea><br>
              {$this->VypisInterpretu("cenik_cena2")}<br>
              cena2:<textarea id=\"cenik_cena2\" name=\"cena2\">{$data->cena2}</textarea><br>
              {$this->VypisInterpretu("cenik_poznamka")}<br>
              poznamky:<textarea id=\"cenik_poznamka\" name=\"poznamka\">{$data->poznamka}</textarea><br>
              <input type=\"submit\" value=\"upravit\" name=\"tlacitko\">
            </form>
          </fileldset>
          ";
    ...
    if ($id != 0)
    {
      if ($res = @$this->var->mysqli->query("SELECT id, sekce, format, cena1, cena2, poznamka FROM cenik WHERE id={$id};"))
      {
        if ($res->num_rows == 1)
        {
          ...
          $result =
          "<p>smazat??
            řádek s id:{$data->id}<br/>
            <a href=\"#\" onclick=\"PoslatAkci('yesdelcenikradek', {$data->id}, 'info_cenik'); return false;\" />ano</a>
            <a href=\"#\" onclick=\"PoslatAkci('', 0, 'info_cenik'); return false;\" />ne</a>
          </p>";
        }
      }
 ...
 */
  function PridejObjednavkuCeniku()
  {
    ...
    {
      $result = "nevyplnili jste potřebné data...";
    }
                                          ...
                                          ORDER BY objednavka_cenik.id ASC"))
    {
      if ($res->num_rows != 0)
      {
        while ($data = $res->fetch_object())
        {
          ...
          $result .=
          "{$data->jmeno} | {$data->prijmeni} | {$data->ulice} | {$data->mesto} | {$data->psc} |
          {$data->telefon} | {$data->email} | {$data->pozadavek} | {$data->datum} | {$ip} | {$host} | {$os} | {$browser} |
          <a href=\"#\" onclick=\"PoslatAkci('zjistizemi', {$ipnum}, 'info_cenik');\">zeme</a> |
          {$data->rozliseni} | {$data->hloubka} | {$data->session} | <a href=\"#\" onclick=\"PoslatAkci('delobjednavka', {$data->id}, 'info_cenik');\">smazat objednávku</a>
          <br/>";
        }
      }
    }
 ...
 */
  function SmazAdminObjednavku($id, $smazat = false)
  {
    ...
    if ($id != 0)
    {
      ...
      "
      smazat id: {$id}??<br>
      <a href=\"#\" onclick=\"PoslatAkci('yesdelobjednavka', {$id}, 'info_cenik'); return false;\">ano</a><br>
      <a href=\"#\" onclick=\"PoslatAkci('', 0, 'info_cenik'); return false;\">ne</a>
      ";
    }
 ...
 *
 * name: SetNastaveni
 * @param jmeno promenne, priznak editace
 * @return vystupni formular
 */
  function SetNastaveni($jmeno, $outelment, $edit = false)
  {
    if (!Empty($jmeno))
    {
      if ($res = @$this->var->mysqli->query("SELECT hodnota FROM nastaveni WHERE promenna='{$jmeno}';"))
 ...
 */
  function VypisBaner()
  {
    if ($res = @$this->var->mysqli->query("SELECT id, cesta, pos_top, pos_left, width, height
                                          FROM banner
      ...
      if ($res->num_rows != 0)
      {
        while ($data = $res->fetch_object())
        {
          ...
          "
              ...
              <span class=\"linka_pod_flashem\" style=\"top: 111px; left: 111px;\"></span>
          ";
        }
      }
    }
 ...
 */
  function VypisAdminBaner()
  {
    ...
    {
      $result .=
      "<a href=\"#\" onclick=\"PoslatAkci('addbaner', 0, 'info_baner');\">přidat baner</a><br/>
      <br>
      ";
      if ($res->num_rows != 0)
      {
        while ($data = $res->fetch_object())
        {
          $result .=
          "{$data->id})
          <a href=\"#\" onclick=\"PoslatAkci('editbaner', {$data->id}, 'info_baner');\">upravit baner</a><br/>
          <a href=\"#\" onclick=\"PoslatAkci('delbaner', {$data->id}, 'info_baner');\">smazat baner</a><br/>
          cesta:{$data->cesta}<br>
          w:{$data->width}<br>
          h:{$data->height}<br>
          t:{$data->pos_top}<br>
 ...
 */
  function PridejAdminBaner($add = false)
  {
    if (!$add)
    {
      ...
      <fileldset>
        <form method=\"post\">
          cesta:<input type=\"text\" id=\"baner_cesta\"><br>
          pos top:<input type=\"text\" id=\"baner_top\"><br>
          pos left:<input type=\"text\" id=\"baner_left\"><br>
          width:<input type=\"text\" id=\"baner_w\"><br>
          height:<input type=\"text\" id=\"baner_h\"><br>
          tlačítko:<input type=\"button\" value=\"přidat\" onclick=\"
          idcesta='baner_cesta';
          idtop='baner_top';
          idleft='baner_left';
      ...
      else
    {
      ...
      {
        if (@$this->var->mysqli->multi_query("INSERT INTO banner (id, cesta, pos_top, pos_left, width, height) VALUES
                                              (NULL, '{$cesta}', {$top}, {$left}, {$w}, {$h});
                                             "))
        {
          $result = "přidan baner: {$cesta}...";
        }
          else
        {
          $this->var->main->ErrorMsg($this->var->mysqli->error);  //chyba do globalni promenne
 ...
 */
  function EditAdmiBaner($id, $edit = false)
  {
    ...
    if ($id != 0)
    {
      if ($res = @$this->var->mysqli->query("SELECT id, cesta, pos_top, pos_left, width, height FROM banner WHERE id={$id};"))
      {
        if ($res->num_rows == 1)
        {
          $data = $res->fetch_object();
      ...
      <fileldset>
        <form method=\"post\">
          cesta:<input type=\"text\" id=\"baner_cesta\" value=\"{$data->cesta}\"><br>
          pos top:<input type=\"text\" id=\"baner_top\" value=\"{$data->pos_top}\"><br>
          pos left:<input type=\"text\" id=\"baner_left\" value=\"{$data->pos_left}\"><br>
          width:<input type=\"text\" id=\"baner_w\" value=\"{$data->width}\"><br>
          height:<input type=\"text\" id=\"baner_h\" value=\"{$data->height}\"><br>
          tlačítko:<input type=\"button\" value=\"upravit\" onclick=\"
          idcesta='baner_cesta';
          idtop='baner_top';
          idleft='baner_left';
    ...
    if ($id != 0)
    {
      if ($res = @$this->var->mysqli->query("SELECT id, cesta, pos_top, pos_left, width, height FROM banner WHERE id={$id};"))
      {
        if ($res->num_rows == 1)
        {
          ...
          $result =
          "<p>smazat??
            nadpis:{$data->cesta}<br/>
            <a href=\"#\" onclick=\"PoslatAkci('yesdelbaner', {$data->id}, 'info_baner'); return false;\" />ano</a>
            <a href=\"#\" onclick=\"PoslatAkci('', 0, 'info_baner'); return false;\" />ne</a>
          </p>";
        }
      }
        else
 ...
 */
  function PoslatHromadnouPostu()
  {
    if ($res = @$this->var->mysqli->query("SELECT login, email FROM uzivatel ORDER BY uzivatel.id"))
    {
      if ($res->num_rows != 0)
      {
        while ($data = $res->fetch_object())
        {
          $result .=
          "<p>
            {$data->login}; email:{$data->email}<br>
          </p>
          ";
          $pro .= "{$data->email}, ";
        }
        ...
        !Empty($_POST["zprava"]))
    {
      ...
      {
        $result = "email se nepodařilo odeslat!";
      }

      $this->AutoClick(1, "ajax.php?action=admin&akce=hrom");
    }
                                          ...
                                          "))
    {
      if ($res->num_rows != 0)
      {
        while ($data = $res->fetch_object())
        {
          ...
          $result .=
          "
          jm: {$data->jmeno} | {$data->pocet} | {$cas} | {$expcas} | {$os} | {$browser} | {$ip} | {$host} | {$data->rozliseni} | {$data->hloubka} |
          <a href=\"#\" onclick=\"PoslatAkci('zjistizemi', {$ipnum}, 'info_poc');\">zeme</a>
          <br>
          ";
                                          ...
                                          "))
    {
      if ($res->num_rows != 0)
      {
        while ($data = $res->fetch_object())
        {
          ...
          $stav = ($data->pristup ? "<span class=\"povoleny_pristup pristup\" title=\"Povolen\">jo</span>" : "<span class=\"nepovoleny_pristup pristup\" title=\"Nepovolen\">ne</span>");
          $dat = strtotime($data->cas);
          $cas = date("H:i:s d.m.Y", $dat);
          ...
          $result .=
          "
          {$login} | {$heslo} | {$stav} | {$cas} | {$expcas} | {$os} | {$browser} | {$ip} | {$host} | {$data->rozliseni} | {$data->hloubka} |
          <a href=\"#\" onclick=\"PoslatAkci('zjistizemi', {$ipnum}, 'info_log');\">zeme</a><br>
          ";
...
<div id=\"zeme_puvodu\">
  ...
  <a href=\"#\" onclick=\"document.getElementById('zeme_puvodu').style.display='none'; return false;\" title=\"Zavřít okno\">zavřit</a>
</div>
    ";
  ...
  function AktualniVyhra()
  {
    if ($res = @$this->var->mysqli->query("SELECT popis FROM vyhry WHERE aktivni=true"))
    {
      if ($res->num_rows == 1)
      {
        ...
        $result = "Vyhra pro tento měsíc: {$popis}...";
      }
        else
      {
        $result = "Žádná vyhra neni pro tento měsíc vybrána";
      }
    }
      else
    {
