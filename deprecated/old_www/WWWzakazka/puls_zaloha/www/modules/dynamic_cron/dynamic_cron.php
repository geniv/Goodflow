...
//verze modulu
define("v_DynamicCron", "2.01");

class DynamicCron extends DefaultModule
{
  private $var, $dirpath, $absolutni_url, $unikatni, $dbpredpona, $mainindex;
  public $idmodul = "dyncron";  //id pro rozliseni modulu v adminu
  private $idlog = "_logy";
  public $mount = array(".unikatni_obsah.php",
                        "../../cron.php");  //corn je pripojeny v korenu!
    ...
    if (!Empty($co))
    {
      switch ($co)
      {
        case "addcron": //pridavani polozky crona
          ...
          $trida = explode(":", $this->NotEmpty("post", "trida"));
          if ($this->ControlForm(array ("modul" => array("self", "string", $trida[0]),
                                        "funkce" => array("self", "string", !Empty($trida[1]) ? $trida[1] : ""),
