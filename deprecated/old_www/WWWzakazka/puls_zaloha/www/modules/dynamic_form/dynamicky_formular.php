...
class DynamicForm extends DefaultModule
{
  ...
  private $input = array ("text"     => "textové pole",  //typ - popis
 ...
 */
  public function __construct(&$var = NULL, $index = -1) //konstruktor
  {
    if (!Empty($var))
    {
      ...
      $this->debug_lokal = $this->unikatni["set_debug_lokal"];  //lokalni zapinani debug modu
 ...
 */
  ...
  {
    if (!$this->InstalaceDatabaze("CREATE TABLE {$this->dbpredpona}formular (
                                  ...
                                  CREATE TABLE {$this->dbpredpona}prvek (
                                    id INTEGER UNSIGNED AUTO_INCREMENT PRIMARY KEY,
 ...
 *
 * pouziti: <strong>$text = $this->var->main[0]->NactiFunkci("DynamicForm", "Formular", "adresa"[, array("pridavek"), 1]);</strong>
 *
 ...
 */
  private function AdministraceObsahu()
  {
    ...
    if (!Empty($co))
    {
      ...
      {
        case "test_rv":
          $vstup = $this->ChangeWrongChar($_POST["vstup"]); //prepinani podle webu/lokalu
          $reg_exp = $this->ChangeWrongChar($_POST["reg_exp"]);  //(!in_array($_SERVER["REMOTE_ADDR"], $this->var->ipblok) ? $_POST["reg_exp"] : )
