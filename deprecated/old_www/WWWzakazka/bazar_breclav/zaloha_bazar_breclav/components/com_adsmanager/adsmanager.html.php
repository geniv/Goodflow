...
class adsmanager_html {
					...
					<td class="adsmanager_table_description">
						...
						while(!$ok)
						{
							if ($i < $conf->nb_images + 1)
							{
								...
								if (file_exists( $pic)) 
								{
									echo "<a href='".$linkTarget."'><img src='".$mosConfig_live_site."/images/$option/ads/".$row->id.$ext_name."_t.jpg' alt='".htmlentities(stripslashes(adsmanager_html::cutLongWord($row->ad_headline)),ENT_QUOTES)."' /></a>";
									$ok = 1;
								}
							}
			...
			<div class="adsmanager_ads_main">
				...
				<div class="adsmanager_ads_image">
					...
					for($i=1;$i < $conf->nb_images + 1;$i++)
					{
						$ext_name = chr(ord('a')+$i-1);
						$pic = $mosConfig_absolute_path."/images/$option/ads/".$row->id.$ext_name."_t.jpg";
						$piclink 	= sefRelToAbs($mosConfig_live_site."/images/$option/ads/".$row->id.$ext_name.".jpg");
						if (file_exists($pic)) 
						{
						    switch($conf->image_display)
						    {
								case 'popup':
									echo "<a href=\"javascript:popup('$piclink');\"><img src='".$mosConfig_live_site."/images/$option/ads/".$row->id.$ext_name."_t.jpg' alt='".htmlentities(stripslashes($row->ad_headline),ENT_QUOTES)."' /></a>";
									break;
								case 'lightbox':
									echo "<a href='".$piclink."' rel='lightbox[roadtrip$row->id]'><img src='".$mosConfig_live_site."/images/$option/ads/".$row->id.$ext_name."_t.jpg' alt='".htmlentities(stripslashes($row->ad_headline),ENT_QUOTES)."' /></a>";
									break;
								case 'default':	
								default:
									echo "<a href='".$piclink."' target='_blank'><img src='".$mosConfig_live_site."/images/$option/ads/".$row->id.$ext_name."_t.jpg' alt='".htmlentities(stripslashes($row->ad_headline),ENT_QUOTES)."' /></a>";
									break;
							}
							$image_found = 1;
						}   
	...
	{		
		...
		function submitbutton(mfrm) {
			...
			if (mfrm.username && (r.exec(mfrm.username.value) || mfrm.username.value.length < 3)) {
				errorMSG += mfrm.username.getAttribute('mosLabel').replace('&nbsp;',' ') + ' : <?php echo addslashes(html_entity_decode(sprintf( _VALID_AZ09, _PROMPT_UNAME, 4 ),ENT_QUOTES)); ?>\n';
				mfrm.username.style.background = "red";
				iserror=1;
			} 
			if (mfrm.password && r.exec(mfrm.password.value)) {
				errorMSG += mfrm.password.getAttribute('mosLabel').replace('&nbsp;',' ') + ' : <?php echo addslashes(html_entity_decode(sprintf( _VALID_AZ09, _REGISTER_PASS, 6 ),ENT_QUOTES)); ?>\n';
				mfrm.password.style.background = "red";
				iserror=1;
			}
			
			if (mfrm.email && !r_email.exec(mfrm.email.value) && mfrm.email.getAttribute('mosReq')) {
				errorMSG += mfrm.email.getAttribute('mosLabel').replace('&nbsp;',' ') + ' : <?php echo html_entity_decode(addslashes(_REGWARN_MAIL),ENT_QUOTES); ?>\n';
		  ...
		  <form action="<?php echo $target;?>" method="post" name="saveForm" enctype="multipart/form-data" onsubmit="return submitbutton(this)">
			...
			if ((!isset($catid))||($catid != 0))
			{
				...
				if (($conf->submission_type == 0)&&($my->id == 0))
				{
					...
					echo "<input $style class='adsmanager_required' mosReq='1' id='username' type='text' mosLabel='".htmlentities(_UNAME,ENT_QUOTES)."' name='username' size='20' maxlength='20' value='$username' /><br />\n"; 
					echo "<label>"._PASS."</label>\n";
					echo "<input $style class='adsmanager_required' mosReq='1' id='password' type='password' mosLabel='".htmlentities(_PASS,ENT_QUOTES)."' name='password' size='20' maxlength='20' value='$password' />\n<br />"; 
					$emailField = false;
					$nameField = false;
					...
					if ($nameField == false)
					{
						...
						echo "<input $style class='adsmanager_required' mosReq='1' id='name' type='text' mosLabel='".htmlentities(_NAME,ENT_QUOTES)."' name='name' size='20' maxlength='20' value='$name' /><br />\n"; 
					}
					if ($emailField == false)
					{
						echo "<label>"._EMAIL."</label>\n";
						echo "<input $style class='adsmanager_required' mosReq='1' id='email' type='text' mosLabel='".htmlentities(_EMAIL,ENT_QUOTES)."' name='email' size='20' maxlength='20' value='$email' /><br />\n"; 
					}
	...
	?>
		...
		<table class="adsmanager_header">
		   <tr>
			  ...
			  <td>
				<?php 
				   $target = sefRelToAbs($mosConfig_live_site."/index.php?option=$option&amp;page=delete_ad&amp;adid=$adid&amp;mode=confirm&amp;Itemid=$itemid");
			  ...
			  <td>
				...
				<?php 
					$target = sefRelToAbs($mosConfig_live_site."/index.php?option=$option&amp;Itemid=$itemid");
	...
	function showProfile($user,$fields,$option,$itemid) {	
		...
		<?php $target = sefRelToAbs($mosConfig_live_site."/index.php?option=$option&amp;page=save_profile&amp;Itemid=$itemid"); ?>
		<form action="<?php echo $target; ?>" method="post" name="mosUserForm">
		<div class="componentheading">
		<?php echo "<div class=\"adsmanager_list_title\" width=\"100%\">"._EDIT_TITLE."</div>"; ?>
		</div>
		<br />
	...
	function recurseCategories( $id, $level, &$children,$itemid,$option) {
		...
		if (@$children[$id]) {
			$i=0;$first=true;
			foreach ($children[$id] as $row) {
				$link = sefRelToAbs("index.php?option=$option&amp;page=show_category&amp;catid=".$row->id."&amp;text_search=&amp;order=0&amp;expand=0&amp;Itemid=".$itemid);
				if ($level == 0)
				{
					if ($i==0)
					{
	...
	?>
		...
		<?php 
			...
			echo '<a href="'.$link_write_ad.'" style="color: #EFD7D7;">'.ADSMANAGER_MENU_WRITE.'</a>';
			echo '';
			echo '';
			echo '';
			echo '';
	...
	function loginpage ($return,$comprofiler) {
		...
		else
		{
			...
			$link_login =sefRelToAbs("index.php?option=com_comprofiler&amp;task=login");
