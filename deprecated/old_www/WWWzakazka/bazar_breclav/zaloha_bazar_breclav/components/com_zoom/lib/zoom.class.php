...
|                                                                     |
-----------------------------------------------------------------------
* @version $Id: zoom.class.php,v 1.113 2006/12/28 22:48:40 kevinuru Exp $
* @package zOOmGallery
...
class zoom {
	...
	* @access public
	*/
	function createdir($path, $mode = '777') {
		global $mosConfig_absolute_path;
		$result = true;
		if ($this->_CONFIG['safemodeON']) {
	...
	function hotlinkimage($cat_id, $type, $imgid, $key) {
        ...
        if($this->_CONFIG['hotlinkProtection']) {
            ...
            if($type == '0') {
                ...
                return $mosConfig_live_site."/".$this->_CONFIG['imagepath'].$gallery->_dir."/thumbs/".$image->_filename;	
            }
        }
	}
	...
	*/
	function optimizeTables() {
		...
		while($rows=mysql_fetch_array($result)){
			...
			$str = $rows['catname'];
			$names[] = preg_replace("/&amp;(#[0-9]+|[a-z]+);/i", "&$1;", htmlspecialchars($str, ENT_QUOTES));
    	    
    	    $str = $rows['catdescr'];
			$descr[] = preg_replace("/&amp;(#[0-9]+|[a-z]+);/i", "&$1;", htmlspecialchars($str, ENT_QUOTES));
    	    
    	    $str = $rows['catkeywords'];
			$keys[] = preg_replace("/&amp;(#[0-9]+|[a-z]+);/i", "&$1;", htmlspecialchars($str, ENT_QUOTES));
		}
	...
	*/
	function getItemid($option) {
		global $database;
		$database->setQuery("SELECT id FROM #__menu WHERE link = 'index.php?option=".$this->escapeString($option)."'");
		if ($this->_result = $database->query()) {
	...
	*/
	function catsArrayToXML($cats, $xml = "") {
	    foreach ($cats as $catid => $data) {
	    	...
	    	$xml .= ("<item text=\"".$data['catname']."\" id=\"".$catid."\" im0=\"folderClosed.gif\" im1=\"folderOpen.gif\""
	    	 . " im2=\"folderClosed.gif\" published=\"".$data['published']."\" keywords=\"".$data['catkeywords']."\""
	    	 . " shared=\"".$data['shared']."\" uid=\"".$data['uid']."\" subcat_id=\"".$data['subcat_id']."\"");
			//$xml .= ("<gallery id=\"".$catid."\" name=\"".$data['catname']."\" subcat_id=\"".$data['subcat_id']."\" pos=\"".$data['pos']."\""
	    	// . " published=\"".$data['published']."\" keywords=\"".$data['catkeywords']."\" shared=\"".$data['shared']."\" uid=\"".$data['uid']."\">");
	    	if (count($data['children']) > 0) {
	    	    $xml .= " open=\"1\" call=\"1\" select=\"1\">".$this->catsArrayToXML($data['children']);
			} else {
			    $xml .= ">";
			}
	    	//$xml .= "</gallery>";
...
//
	...
	*/
	function adminFooter() {
		?>
		<p align="center">
			<b>zOOm&nbsp;Media&nbsp;Gallery&nbsp;<?php echo $this->_CONFIG['version'];?></b><br />Copyright &copy; 2003 - 2006 by Mike de Boer.</p>
		<?php
	}
	/**
	* @return string
