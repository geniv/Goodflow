 ...
 */
  ...
  {
    $result =
    "<a href=\"?{$this->var->get_kam}={$this->var->adresaadminu}&amp;{$this->var->get_idmodul}={$this->idmodul}\" title=\"\">administrace dynamickeho menu</a><br />";
    ...
    if (!Empty($co))
    {
      switch ($co)
      {
        case "addgrup": //pridavani skupin
          ...
          <form method=\"post\">
            <fieldset>
              adresa: <input type=\"input\" name=\"adresa\" value=\"{$_GET["adresa"]}\" />(url adresa v ktere se menu nachazi... z parametru a nebo \$_SERVER[\"QUERY_STRING\"])<br />
              nazev: <input type=\"input\" name=\"nazev\" /><br />
              poradi: <input type=\"input\" name=\"poradi\" value=\"{$this->PocetSkupin(1)}\" /> >0<br />
              razeni:<br />
        ...
        case "editgrup":  //uprava
          ...
          if ($res = @$this->sqlite->query("SELECT nazev, poradi, razeni, adresa FROM skupina_menu WHERE id={$id};", NULL, $error))
          {
            if ($res->numRows() == 1)
            {
              ...
              <form method=\"post\">
                <fieldset>
                  adresa: <input type=\"input\" name=\"adresa\" value=\"{$data->adresa}\" />(url adresa v ktere se menu nachazi... z parametru a nebo \$_SERVER[\"QUERY_STRING\"])<br />
        ...
        case "addmenu": //pridavani poloze menu
          ...
          <form method=\"post\">
            <fieldset>
              ...
              ".($this->VolbaRazeni($skup) ? "poradi: <input type=\"input\" name=\"poradi\" value=\"{$this->PocetRadku($skup, 1)}\" /><br />" : "")."
            ...
            if ($res->numRows() == 1)
            {
              ...
              <form method=\"post\">
                <fieldset>
                  ...
                  ".($this->VolbaRazeni($data->skupina) ? "poradi: <input type=\"input\" name=\"poradi\" value=\"{$data->poradi}\" /><br />" : "")."
