    ...
    if ($res = @$this->sqlite->query("SELECT popisek, url FROM picture_gallery WHERE skupina={$sekce} ORDER BY {$order};", NULL, $error))
    {
      if ($res->numRows() != 0)
      {
        ...
        while ($data = $res->fetchObject())
        {
          ...
          $ente = ((($i + 1) % 3) == 0);  //kazde 3 od 1
          $ente1 = ((($i + 0) % 3) == 0);  //kazde 3 od 0

          $result .=
          "  <a href=\"".($this->var->htaccess ? "../" : "")."{$this->dirpath}/{$this->pathpicture}/{$this->fulldir}/{$data->url}\" class=\"highslide galerie_v_sekcich".($ente ? "_posledni" : ($ente1 ? "_prvni" : ""))."\" title=\"{$data->popisek}\" onclick=\"return hs.expand(this)\">
    <img src=\"".($this->var->htaccess ? "../" : "")."{$this->dirpath}/{$this->pathpicture}/{$this->minidir}/{$data->url}\" alt=\"{$data->popisek}\" />
 ...
 */
  ...
  {
    $result =
    "<a href=\"?{$this->var->get_kam}={$this->var->adresaadminu}&amp;{$this->var->get_idmodul}={$this->idmodul}\" title=\"\">administrace dynamické obrázkové galerie</a><br />";
    ...
    if (!Empty($co))
    {
      switch ($co)
      {
        case "config":  //kofigurace galerie
          ...
          <form method=\"post\">
            <fieldset>
              ...
              width mini: <input type=\"input\" name=\"w_mini\" id=\"mini1_p1\" value=\"{$w_mini}\" /> px<br />
              <br />
              <input type=\"radio\"".($h_mini != 0 ? " checked=\"checked\"" : "")." name=\"mini\" onclick=\"mini_2();\">dopocitat sirku<br />
              height mini: <input type=\"input\" name=\"h_mini\" id=\"mini2_p1\" value=\"{$h_mini}\" /> px<br />
              <br />
              <input type=\"radio\"".($w_mini != 0 && $h_mini != 0 ? " checked=\"checked\"" : "")." name=\"mini\" onclick=\"mini_3();\">nastaveno napevno<br />
              width mini: <input type=\"input\" name=\"w_mini\" id=\"mini3_p1\" value=\"{$w_mini}\" /> px<br />
              height mini: <input type=\"input\" name=\"h_mini\" id=\"mini3_p2\" value=\"{$h_mini}\" /> px<br />
              <br />
              <br />
              full:<br />
              <input type=\"radio\"".($w_full != 0 ? " checked=\"checked\"" : "")." name=\"full\" onclick=\"full_1();\">dopocitat vysku<br />
              width full: <input type=\"input\" name=\"w_full\" id=\"full1_p1\" value=\"{$w_full}\" /> px<br />
              <br />
              <input type=\"radio\"".($h_full != 0 ? " checked=\"checked\"" : "")." name=\"full\" onclick=\"full_2();\">dopocitat sirku<br />
              height full: <input type=\"input\" name=\"h_full\" id=\"full2_p1\" value=\"{$h_full}\" /> px<br />
              <br />
              <input type=\"radio\"".($w_full != 0 && $h_full != 0 ? " checked=\"checked\"" : "")." name=\"full\" onclick=\"full_3();\">nastaveno napevno<br />
              width full: <input type=\"input\" name=\"w_full\" id=\"full3_p1\" value=\"{$w_full}\" /> px<br />
              height full: <input type=\"input\" name=\"h_full\" id=\"full3_p2\" value=\"{$h_full}\" /> px<br />
              <br />
              <input type=\"radio\"".($w_full == 0 && $h_full == 0 ? " checked=\"checked\"" : "")." name=\"full\" onclick=\"full_4();\">originální velikost<br />
              width full: <input type=\"input\" name=\"w_full\" id=\"full4_p1\" value=\"0\" disabled=\"disabled\" /> px<br />
              height full: <input type=\"input\" name=\"h_full\" id=\"full4_p2\" value=\"0\" disabled=\"disabled\" /> px<br />
              <br />
              <br />
              limit:<br />
              <input type=\"radio\"".($limit != 0 ? " checked=\"checked\"" : "")." name=\"lim\" onclick=\"lim_1();\">dany limit<br />
              limit upload: <input type=\"input\" name=\"limit\" id=\"lim_p1\" value=\"{$limit}\" /> MB<br />
              <br />
        ...
        case "addgrup": //pridavani skupiny
          ...
          <form method=\"post\">
            <fieldset>
              nazev: <input type=\"input\" name=\"nazev\" onkeyup=\"PrepisovaniTextu(this.value)\" /><br />
              rewrite: <input type=\"input\" id=\"rewrite_input\" name=\"rewrite\" readonly=\"readonly\" /><br />
              popis: <input type=\"input\" name=\"popis\" /><br />
              datum: <input type=\"input\" name=\"datum\" value=\"".date("j.n.Y H:i:s")."\" /><br />
        ...
        case "editgrup":  //editace skupiny
          ...
          if ($res = @$this->sqlite->query("SELECT nazev, rewrite, popis, datum, defaultni FROM skupina WHERE id={$id};", NULL, $error))
          {
            if ($res->numRows() == 1)
            {
              ...
              <form method=\"post\">
                <fieldset>
                  nazev: <input type=\"input\" name=\"nazev\" value=\"{$data->nazev}\" onkeyup=\"PrepisovaniTextu(this.value)\" /><br />
                  rewrite: <input type=\"input\" id=\"rewrite_input\" name=\"rewrite\" readonly=\"readonly\" value=\"{$data->rewrite}\" /><br />
                  popis: <input type=\"input\" name=\"popis\" value=\"{$data->popis}\" /><br />
                  datum: <input type=\"input\" name=\"datum\" value=\"".date("j.n.Y H:i:s", strtotime(stripslashes($data->datum)))."\" /><br />
        ...
        case "addpict": //pridavani obrazku
          ...
          <form method=\"post\" enctype=\"multipart/form-data\">
            <fieldset>
              popisek: <input type=\"input\" name=\"popisek\" /><br />
        ...
        case "editpict":  //editace skupiny
          ...
          if ($res = @$this->sqlite->query("SELECT popisek, url, datum, skupina FROM picture_gallery WHERE id={$id};", NULL, $error))
          {
            if ($res->numRows() == 1)
            {
              ...
              <form method=\"post\" enctype=\"multipart/form-data\">
                <fieldset>
                  popisek: <input type=\"input\" name=\"popisek\" value=\"{$data->popisek}\" /><br />
