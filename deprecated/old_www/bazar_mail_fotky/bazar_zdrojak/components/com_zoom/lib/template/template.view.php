...
|                                                                     |
-----------------------------------------------------------------------
* @version $Id: template.view.php,v 1.14 2006/12/28 23:13:34 kevinuru Exp $
* @package zOOmGallery
* @subpackage Template
...
class ZMG_Template_View {
    ...
    function showTypeImage($use_tables = true, $img_path, $fullsize, &$viewsize_info, $key) {
        ...
        if ($use_tables) {
            ...
            ?>
            <div class="zmg-image-full-outer">
                <div class="img-shadow-full-border">
                    <div class="img-shadow-full">
                    	<div id="zImageBox">
                        	<div style="float:center" <?php echo ($zoom->_CONFIG['zoomOn']) ? "onmouseover=\"tjpZoom.on(event,".$viewsize_info[0].",".$viewsize_info[1].",'".$img_path."'".$fullsize.");\" onmousemove=\"tjpZoom.move(event);\" onmouseout=\"tjpZoom.off();\"" : ""; ?>>
    ...
    function showTypeDocument($thumbnail, $popup, $img_path) {
        ?>
		<a href="<?php echo $img_path; ?>"><img src="<?php echo $thumbnail; ?>" alt="" border="1" name="zImage" /></a>
		<?php
    }
        ...
        if (empty($zoom->_gallery->_images[$key]->_comments)) {
            ...
            foreach ($zoom->_gallery->_images[$key]->_comments as $comment) {
                ...
                if ($use_tables) {
                    ...
                    $cmtrow = "<div class=\"zmg-comment-block\">\n<div class=\"comments-added-date\">\n".$comment->getDate()."</div>\n<div class=\"zmg-comments-added-name\"><img src=\"".$dir_prefix."images/comment.png\" border=\"0\" title=\"Comment\" alt=\"Image of comment\" />".$comment->getName()." <span class=\"zmg-comments-says\">says:</span> </div>\n<div class=\"zmg-comments-added-text\">".$comment->getComment($dir_prefix)."</div>\n</div>\n";
                }
                if ($zoom->_isAdmin) {
                	if ($use_tables) {
                		$cmtrow .= "<td width=\"50\">";
