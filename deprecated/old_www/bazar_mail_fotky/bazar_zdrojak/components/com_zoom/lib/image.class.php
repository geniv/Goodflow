...
|                                                                     |
-----------------------------------------------------------------------
* @version $Id: image.class.php,v 1.27 2006/12/23 18:12:15 kevinuru Exp $
* @package zOOmGallery
...
class image extends gallery {
	...
	function setImgInfo($filename="", $name, $keywords, $descr, $catid=0, $uid=0, $published=0, $selections=1) {
	    ...
	    $this->_descr = str_replace("'", '&#39;', $zoom->htmlnumericentities($descr));
	...
	*/
	...
	function _drawStars($votes, $points) {
		...
		<li class="current-rating" style="width:'.(@number_format($rate,2)*15).'px;"><span class="hide">'.@number_format($rate,2).'</span></li>';
		for ($ncount = 1; $ncount <= 5; $ncount++) {
			$html .= '<li><span title="'.constant('_ZOOM_RATE'.$ncount).'" class="r'.$ncount.'-unit"><span class="hide">'.$ncount.'</span></span></li>';
        }
	...
	function _drawVotingStars($votes, $points) {
		...
		<li class="current-rating" style="width:'.(@number_format($rate,2)*15).'px;"><span class="hide">'.@number_format($rate,2).'</span></li>';
		for ($ncount = 1; $ncount <= 5; $ncount++) {
			$html .= '<li><a href="javascript:Zoom.unit_save_vote(\''.$this->_id.'\', \''.$ncount.'\')" title="'.constant('_ZOOM_RATE'.$ncount).'" class="r'.$ncount.'-unit"><span class="hide">'.$ncount.'</span></a></li>';
        }
		$ncount = 0; // resets the count
		$html .= '</ul>
		<strong> '.@number_format($rate, 2).'</strong>/ '.$votes.' '.(($this->_votenum == 1) ? _ZOOM_VOTE : _ZOOM_VOTES).'
