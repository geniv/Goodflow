...
|                                                                     |
-----------------------------------------------------------------------
* @version $Id: mediamgr.php,v 1.13 2006/12/23 04:30:19 kevinuru Exp $
* @package zOOmGallery
...
if ($task == 'edtimg' && $zoom->privileges->hasPrivilege('priv_editmedium')) {
	...
	<?php
	if ($zoom->_isBackend) {
		$mainlink = "index".$backend.".php?option=com_zoom&amp;Itemid=".$Itemid."&amp;page=admin";
	} else{
		$mainlink = sefReltoAbs("index.php?option=com_zoom&amp;Itemid=".$Itemid."&amp;page=admin");
	}
	?>
		<td align="center" width="100%"><a href="<?php echo $mainlink ?>">
