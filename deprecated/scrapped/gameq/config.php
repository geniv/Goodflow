...
class Config {
    ...
    private function readIniFile($path) {
        ...
        if (count($data) == 0) {
            $msg = sprintf('Config::readIniFile: unable to read file [%s].', $path);
    ...
    public function getGame($gid, $addr, $port) {
        ...
        if (!array_key_exists($gid, $this->games)) {
            $msg = sprintf('Config::getGame: Unknown game identifier [%s].', $gid);
            trigger_error($msg, E_USER_ERROR);
        }
        $game = $this->games[$gid];
    ...
    public function getPackets($pid) {
        if (!array_key_exists($pid, $this->packets)) {
            $msg = sprintf('Config::getPackets: Unknown packet identifier [%s].', $pid);
            trigger_error($msg, E_USER_ERROR);
        }
