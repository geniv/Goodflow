...
class DynamicMenuWithoutGroup extends DefaultModule
{
  private $var;
  private $sqlite;
  private $dbname;
  private $idmodul = "dynmenuoutgrup";  //id pro rozliseni modulu v adminu
  private $dirpath;
 ...
 */
  public function __construct(&$var, $index) //konstruktor
  {
    ...
    $adresa_menu = array( array("main_href" => "{$this->idmodul}",
                                "odkaz" => "administrace dynamickeho menu bez skupin",
                                "title" => " - administrace dynamickeho menu bez skupin",
                                "id" => "",
                                "class" => "",
                                "akce" => ""),
                          );
    ...
    if ($res = @$this->sqlite->query("SELECT main_href, href, odkaz, href_id, href_class, href_akce FROM dynamic_menu_without_group WHERE adresa='{$adresa}' ORDER BY poradi ASC;", NULL, $error))
    {
      if ($res->numRows() != 0)
      {
        ...
        while ($data = $res->fetchObject())
        {
          ...
          $result .=
           "\n          <p{$class}>
                <a href=\"{$mainhref}{$href}\" title=\"{$data->odkaz}\"{$id}{$class}{$akce}>
                  <span>
