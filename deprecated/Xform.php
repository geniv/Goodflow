...
//~ SUBMITTED = ':submitted';
//TODO doaplikovat zbyle rule checkry
//~ // file upload
...
//var_dump($method, $parameters);
      ...
      if (is_string($settings)) {
        $lab = array('label' => $settings);
        $settings = $lab;
        //~ $settings = array('label' => $settings);  //TODO reimplementovat
      }
...
//var_dump($settings);
      ...
      if (!$name) { //overovani prazdnoty jmena
        throw new ExceptionLightHtml('name for: '.$method.' is empty!!');
      }
      ...
      switch ($method) {
        ...
        case 'addDatalist': //addDatalist($name, $settings = array())
          //TODO + TEST podobny prvek jako select, az na to ze se datalist vaze na input
          //pres ID datalistu!
        break;
        ...
        default:
          throw new ExceptionLightHtml('unknown element!');
        break;
      }

      if (!is_null($element)) { //create self instance Control
        //budovani obalu z callback funkci
        $label_callback_data = array('html' => $html, 'label' => $label, 'element' => $element);
...
//TODO udelat verzi pro formcontrol (setDefaultValue)
    ...
    private function _isSubmitted() {
      $values = $this->getMethodValues(); //TODO mozna case trochu poupravi
      return (!empty($values));
    }
    ...
    public function isValid() {
      ...
      $separe_element = function($val, $key, $data) {
        $data['result'][] = $val->checkValidateRule($key, $val);
        if ($val->hasErrors()) {
...
//TODO nastavovani odesilaciho tlacitka
    ...
    public function render() {
      ...
      $form = $htmlClass::form($this->form->attribute);
