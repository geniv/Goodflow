...
*/
    ...
    function addProject() {
        ...
        if ($('#project_name').val().length > 0) {
            $.post('{$weburl}ajax.php', {'type': 'addproject', 'name': $('#project_name').val()}, function(data) {
    ...
    function editProject(id, value) {
        ...
        if (name != null && name != value) {  // pokud je nazev lisi a nezmackl se cancel
            $.post('{$weburl}ajax.php', {'type': 'editproject', 'id': id, 'name': name}, function(data) {
    ...
    function delProject(id, value) {
        if (confirm('opravdu smazat projekt: ' + value)) {
            $.post('{$weburl}ajax.php', {'type': 'delproject', 'id': id}, function(data) {
    ...
    function restoreProject(id, value) {
        if (confirm('opravdu obnovit projekt: ' + value)) {
            $.post('{$weburl}ajax.php', {'type': 'restoreproject', 'id': id}, function(data) {
                // console.log(data);
                window.location.reload();
            });
        }
    ...
    function archiveProject(id, value) {
        if (confirm('opravdu archivovat projekt: ' + value)) {
            $.post('{$weburl}ajax.php', {'type': 'archiveproject', 'id': id}, function(data) {
                // console.log(data);
                window.location.reload();
            });
        }
    ...
    // pridani tasku
    function addTask(id) {
        $.post('{$weburl}ajax.php', {'type': 'addtask', 'id': id}, function(data) {
    ...
    function delTask(id, task, value, time) {
        ...
        if (confirm('opravdu smazat task z projektu: ' + value + '  započatý v: ' + time)) { /// t.getDate() + (t.getMonth()+1) )
            $.post('{$weburl}ajax.php', {'type': 'deltask', 'id': id, 'task': task}, function(data) {
    ...
    function usedTask(id, task, state) {
        $.post('{$weburl}ajax.php', {'type': 'usedtask', 'id': id, 'task': task, 'state': state}, function(data) {
            // console.log(data);
        });
    }
    ...
    function stopTask(id, task, value, time) {
        if (confirm('opravdu zastavit task z projektu: ' + value + ' započatý v: ' + time)) {
            $.post('{$weburl}ajax.php', {'type': 'stoptask', 'id': id, 'task': task}, function(data) {
    ...
    function pauseTask(id, task) {
        $.post('{$weburl}ajax.php', {'type': 'pausetask', 'id': id, 'task': task}, function(data) {
    ...
    function saveDescription(id, task, value) {
        ...
        tim = setTimeout(function() {
            $.post('{$weburl}ajax.php', {'type': 'savedescription', 'id': id, 'task': task, 'description': value}, function(data) {
    ...
    setInterval(function() {  // casova smycka
        $.post('{$weburl}ajax.php', {'type': 'activetask'}, function(data) {
            // console.log(data);
