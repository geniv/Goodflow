...
//TODO doresit nejak synchronizaci s .map??
  ...
  //~ var_dump($codeSplit);
      ...
      if ($codeSplit) {
        ...
        foreach ($codeSplit as $html) {
          ...
          if (preg_match($tagMatch['loop'], $html, $match)) {
            ...
            $result .= '\'; '.$counter.' =- 1; '.$_array.' = '.$var.'; if (isset('.$_array.') && (is_array('.$_array.') || '.$_array.' instanceof \Traversable) && !empty('.$_array.')) foreach ('.$_array.' as '.$key.' => '.$value.') { '.$counter.'++; '.$cmrs.' .= \'';
          ...
          if (preg_match($tagMatch['date'], $html, $matches)) {
            ...
            $result .= '\'.(date("'.($format ?: 'Y-m-d H:i:s').'"'.($timestamp ? ','.$timestamp : '').')).\'';
          } else
     ...
     * @return zpracovany html
     */
    private function replaceTranslate($html) {
      return '\'.gettext("'.$html.'").\'';
    }
