 ...
 */
  ...
  class PDODatabase {
    ...
    public function execSQL($sql, $param = array()) {
      $result = NULL;
      $result = $this->pdo->exec(!empty($param) && is_array($param) ? vsprintf($sql, $param) : $sql);
      $this->checkPDOError();
      return $result;
    }
  ...
  class ContentValues {
     ...
     */
    public function __construct(ContentValues $from = null) {
      $this->content = array();
      if (!is_null($from)) {
   ...
   *
   */
  class PDOCursor implements \Countable, \Iterator {
    const VERSION = 1.88;
...
//FIXME dporuceni: nepouzivat zatim metody pracujici s fetchAll !!!!! jsou tu kvuli ni duplikace kodu!
   ...
   *
   */
  class PDOCursorData implements \ArrayAccess {
    const VERSION = 1.12;
