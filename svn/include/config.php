...
//
// $config->setSVNCommandPath('/www/svn'); //  e.g. c:\\program files\\subversion\\bin
...
foreach ($it as $row) {
    if ($row->isDir() && !$row->isDot()) {
        ...
        $init = new \DirectoryIterator($svnpath . '/' . $dir);
        foreach ($init as $r) {
            if ($r->isDir() && !$r->isDot()) {
                $dirs[] = $dir . '/' . $r->getFilename();
            }
        }
    }
}
...
foreach ($dirs as $dir) {
	$config->addRepository(basename($dir), 'file://' . $svnpath . '/' . $dir);
}

// $config->addRepository('pokusX2', 'http://www.gfdesign.cz/svn/apps/pokusX2');

// $config->addRepository('pokus1', 'http://www.gfdesign.cz/svn/websites/pokus1');
// $config->addRepository('pokus2', 'http://www.gfdesign.cz/svn/websites/pokus2');
...
// Local repositories (without and with optional group):
//
// $config->addRepositorySubpath('NameToDisplay', 'file://svn', 'subpath');
...
// $config->useAuthenticationFile('/www/svn/.htpasswd', 'My WebSVN Realm'); // Global access file
