...
//TODO dopsat testy!!!!
  ...
  final class Paginator {
    ...
    /**
     * schovany defaultni konstruktor
     *
     * @since 2.00
     * @param void
     */
    ...
    const TYPE1 = 1;  /** zkracene zobrazovani s prvnim - () 1 ... 4 5 6 ... 9 () */
    const TYPE2 = 2;  /** zkracene zobrazovani bez prvniho - () 4 5 6 () */
    const TYPE3 = 3;  /** zkracene zobrazovani bez prvniho - () 4 5 6 7 8 () */
    ...
    public function render($type = null, $settings = null) {
      ...
      switch ($type) {
        ...
        case self::TYPE2:
          ...
          if ($page < $koef) {
            $result = array_merge($result, range(1, $koef));
          } else
          if ($page >= $koef && $page <= $last - ($koef - 1)) {
            $result = array_merge($result, range($page - $range, $page + $range));
          } else
          if ($page > $last - ($koef - 1)) {
            $result = array_merge($result, range($last - ($koef - 1), $last));
          }
        break;
      }
      return $result;
