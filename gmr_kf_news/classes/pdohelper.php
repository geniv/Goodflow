 ...
 */
  ...
  abstract class PDOHelper {
     ...
     * @return this
     */
    private function __SQLiteX($type, $dbpath = null) {
    ...
    public function SQLite3($dbpath = null) {
      return $this->__SQLiteX('sqlite', $dbpath);
    }
    ...
    public function getDatabase($autocreate = false) {
      ...
      if ($this->handle) {
        ...
        if ($autocreate) {
          static::onCreate($this->handle);  //volani onCreate u potomka
        }
        return $this->handle;
      } else {
  ...
  final class PDODatabase {
    ...
    private function _insert($table, ContentValues $values) {
      ...
      $keys = implode(', ', $content['keys']);
      $values = implode(', ', array_fill(0, count($content['keys']), '?'));
  ...
  final class PDOCursor extends \PDOStatement {
     ...
     * @since 2.30
     * @param void
     * @return PDOCursorData hodnota polozky
     */
