...
// pole pro uložení informací o notách
...
$(function(){	
	...
	$(document).keydown(function(e){
		...
		{
			// stisk klávesy ";" (186) zobrazí uložená data
			if (e.which == 186)
			{
...
function playMusic()
{
	// zahájení přehrávání melodie a basové linky
...
// herní smyčka spouštěná každých 30 milisekund
function gameloop()
{
	var game = document.getElementById("game-canvas");
	var ctx = game.getContext('2d');
	
	// pokud je hra spuštěná, zobrazí se nové body
	...
	for(var i in audiogame.dots)
	{
		if (!audiogame.dots[i].missed && audiogame.dots[i].distance < -10)
		{
			...
			// snížení počtu úspěšných zásahů
			audiogame.successCount--;
			
			// počet úspěšných zásahů nemůže být záporný
			audiogame.successCount = Math.max(0, audiogame.successCount);
									
		}
		
		// odstranění nezasažených bodů
	...
	// výpočet úspěšnosti za posledních 5 bodů
	var successPercent = audiogame.successCount / 5;
	
	// úspěšnost nesmí vybočit z rozmezí 0 až 1
	successPercent = Math.max(0, Math.min(1, successPercent));	
	
	// nastavení hlasitosti melodie na základě úspěšnosti
  ...
  // vymaže se pouze ta část plátna, jejíž obsah se změnil, 
  // tzn. prostřední část
	ctx.clearRect(ctx.canvas.width/2-200, 0, 400, ctx.canvas.height);
				
	
	// zobrazení hudebních bodů
	for(var i in audiogame.dots)
	{
		
		// příprava výplně radiálním gradientem		
		...
		// zobrazení bodu na určené pozici
...
// obsluha události konce přehrávání audia
function onMelodyEnded()
{
	console.log('skladba skončila');
	console.log('úspěšnost: ',audiogame.totalSuccessCount / audiogame.totalDotsCount * 100 + '%');
}