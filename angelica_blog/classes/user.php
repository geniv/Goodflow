 ...
 */
  ...
  class Identity implements IIdentity {
    ...
    public function getData($index = null) {
      return ($index ? (isset($this->data[$index]) ? $this->data[$index] : null) : $this->data);
    }
  }
  ...
  class User {
    ...
    /**
     * defaultni konstruktor a vytvoreni session a session sekce
     *
     * @since 2.00
    ...
    /**
     * tovarni metoda na vytvoreni nejcastejsi variaty uzivani
     *
     * @since 2.56
     * @param string session_name nazev session sekce
     ...
     * @return User vytvorena instance
     */
    public static function create($session_name, IAuthenticator $handler, $time) {
      $user = new self($session_name);
      return $user->setAuthenticator($handler)->setExpiration($time);
    }
    ...
    public function login($identity, $password = null) {
      if (!($identity instanceof IIdentity)) {
        $identity = $this->authenticator->authenticate(array($identity, $password));  // nacteni identity z authenticatoru
      }

      if (!is_null($identity)) {  // pokud je identita dodana
     ...
     * - musi byt identita, musi byt autenticovan a musi byt stejny client info
     * - pokud je autentikovan a neexistuje identita jako by nebyl prihlaseny!
     *
     * @since 2.00
     * @param void
    ...
    public function isLoggedIn() {
      return ($this->getSessionIdentity() && $this->isSessionAuthenticated() && $this->getSessionClientInfo() === $this->getServerClientInfo());
    }
  ...
  class StaticACL implements IAuthorizator {
     ...
     * @since 4.00
     * @param string|null path cesta pro soubor
     * @param bool force true pro urgentni ulozeni
     * @return this
     */
    public function commitRules($path = null, $force = false) {
      // pokud se nenacita ze souboru, provadi zavislosti
      if (!$this->loadFromFile || $force) {
     ...
     * - pokud neni definovano allow je automaticky false
     * - all vyhodnocuje az posledni
     *
     * @since 4.00
     * @param string role nazev role
     * @param string resource nazev zdroje
